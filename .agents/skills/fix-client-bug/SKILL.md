---
name: fix-client-bug
description: Fix bugs or add features in the @haneullabs/haneul client layer (gRPC, JSON-RPC, GraphQL). Ensures all three transport implementations stay in sync.
user-invocable: true
...
Use this workflow when making changes to Core API methods in `packages/haneul/src/`. The key architectural constraint is that the three transport implementations (gRPC, JSON-RPC, GraphQL) must always produce identical results for the same Core API call.

## Phase 1: Read Before Writing

1. Identify the affected Core API method(s) in `packages/haneul/src/client/core.ts`.
2. Read the shared return types in `packages/haneul/src/client/types.ts`.
3. Read **all three** transport implementations of the method:
   - `packages/haneul/src/grpc/core.ts`
   - `packages/haneul/src/jsonRpc/core.ts`
   - `packages/haneul/src/graphql/core.ts`
...
- Use explicit return type annotations in the mapping functions (e.g., `(item): HaneulClientTypes.SomeType => { ... }`) to catch type mismatches at the implementation site.
...
- Regenerate types: `pnpm --filter @haneullabs/haneul codegen:graphql`
...
1. Add or update tests in `packages/haneul/test/e2e/clients/core/`.
...
1. Build: `pnpm turbo build --filter=@haneullabs/haneul`
2. Lint: `pnpm --filter @haneullabs/haneul oxlint:check`
3. Unit tests: `pnpm --filter @haneullabs/haneul test`
