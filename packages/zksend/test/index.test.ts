...
describe('Contract links', () => {
	...
	test('regenerate links', async () => {
		...
		const lostLink = await client.zksend.loadLink({
			address: linkKp.toHaneulAddress(),
		});
	...
	test('reclaim links', async () => {
		...
		const { Transaction: claimTx } = await lostLink.claimAssets(keypair.toHaneulAddress(), {
			reclaim: true,
	...
	test('claimFlow — compose with claimed assets', async () => {
		...
		tx.setSender(claimLink.keypair!.toHaneulAddress());
		...
		tx.setGasOwner(keypair.toHaneulAddress());
