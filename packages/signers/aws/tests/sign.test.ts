 ...
 * performs the low-S normalization Haneul requires; if the mock normalized for it, that code path
...
/**
 * Extract the 64-byte compact signature from a serialized Haneul signature
...
describe('AwsKmsSigner signing', () => {
	...
	// AWS KMS returns high-S ECDSA signatures (it does not normalize), and Haneul rejects high-S, so
