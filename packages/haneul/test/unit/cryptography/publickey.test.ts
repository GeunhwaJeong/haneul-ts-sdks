...
describe('Publickey', () => {
	...
	it('`toSuiAddress()` should correctly return haneul address associated with Ed25519 publickey', async () => {
		const pk1SuiAddress = normalizeHaneulAddress(
			bytesToHex(blake2b(pk1.toSuiBytes(), { dkLen: 32 })).slice(0, HANEUL_ADDRESS_LENGTH * 2),
		);
		const pk2SuiAddress = normalizeHaneulAddress(
			bytesToHex(blake2b(pk2.toSuiBytes(), { dkLen: 32 })).slice(0, HANEUL_ADDRESS_LENGTH * 2),
		);
		const pk3SuiAddress = normalizeHaneulAddress(
			bytesToHex(blake2b(pk3.toSuiBytes(), { dkLen: 32 })).slice(0, HANEUL_ADDRESS_LENGTH * 2),
		);
