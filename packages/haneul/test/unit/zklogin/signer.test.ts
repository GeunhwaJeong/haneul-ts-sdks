...
import { bcs, fromBase64, toBase64 } from '@haneullabs/bcs';
...
describe('ZkLoginSigner', () => {
	...
	test('derives the current (non-legacy) address from the proof', () => {
		...
		expect(signer.getPublicKey().toHaneulAddress()).toBe(expected);
		expect(signer.toHaneulAddress()).toBe(expected);
	});

	test('derives the legacy address from the proof when legacyAddress is true', () => {
		const signer = makeSigner(true);
		expect(signer.getPublicKey().toHaneulAddress()).toBe(anAddress);
	});
	...
	test('propagates legacyAddress into derivation (legacy and current differ for a short seed)', () => {
		...
		expect(legacy.getPublicKey().toHaneulAddress()).not.toBe(current.getPublicKey().toHaneulAddress());
		expect(legacy.getPublicKey().toHaneulAddress()).toBe(
			toZkLoginPublicIdentifier(seed, iss, { legacyAddress: true }).toHaneulAddress(),
		);
		expect(current.getPublicKey().toHaneulAddress()).toBe(
			toZkLoginPublicIdentifier(seed, iss, { legacyAddress: false }).toHaneulAddress(),
		);
	});
