...
describe('tx.balance', () => {
	...
	it('gas type tx.balance: no remainder handling (GasCoin must remain)', async () => {
		const tx = new Transaction();
		tx.setSenderIfNotSet(SENDER);
		const bal = tx.balance({ type: '0x2::haneul::HANEUL', balance: 50n });
		tx.moveCall({
			target: '0x2::balance::send_funds',
	...
	it('tx.coin with useGasCoin: false treats HANEUL as regular coin type', async () => {
		...
		const tx = new Transaction();
		tx.setSenderIfNotSet(SENDER);
		tx.transferObjects([tx.coin({ type: HANEUL_TYPE, balance: 50n, useGasCoin: false })], RECEIVER);
		...
		const result = await resolvedData(
			...
			mockClient({
				addressBalance: 0n,
				coinBalance: 100n,
				coins: [makeCoin('0xa1', '100', HANEUL_TYPE)],
			}),
		);

		// Should NOT use GasCoin — should split from the HANEUL coin object
	...
	return {
		core: {
			getBalance: async ({ coinType }: { coinType?: string } = {}) => ({
				balance: {
					coinType: normalizeStructTag(coinType ?? '0x2::haneul::HANEUL'),
