...
describe('coinWithBalance', () => {
	...
	/** Get a fresh funded signer with HANEUL and TEST coins for isolated test runs. */
	...
	testWithAllClients('works with haneul', async (client) => {
		const { keypair, address } = await toolbox.getSigner({ coins: [1_000_000_000n] });
	...
	testWithAllClients('works with custom coin', async (client) => {
		...
		expect(
			result.Transaction!.balanceChanges?.find(
				(change) => change.address === receiver.toHaneulAddress(),
			),
		).toEqual({
			amount: '1',
			coinType: testType,
		...
		expect(
			result.Transaction?.effects.changedObjects?.filter((change) => {
				...
				return (
					...
					change.outputOwner.AddressOwner === receiver.toHaneulAddress()
				);
			}).length,
		).toEqual(1);
	});
	...
	testWithAllClients('works with multiple coins', async (client) => {
		...
		expect(
			...
			{
				amount: '7',
				coinType: normalizeStructTag('0x2::haneul::HANEUL'),
				address: receiver.toHaneulAddress(),
			},
			{
				amount: '3',
	...
	describe('with address balance', () => {
		testWithAllClients('uses address balance for HANEUL when available', async (client) => {
			...
			async (client) => {
				...
				mintTx.moveCall({
					...
					arguments: [
						mintTx.object(treasuryCapId),
						mintTx.pure.u64(50),
						mintTx.pure.address(signer.toHaneulAddress()),
					],
				});
	...
	describe('createBalance', () => {
		...
		beforeAll(async () => {
			...
			// Fund both with HANEUL + mint TEST coins
			const setupTx = new Transaction();
			setupTx.transferObjects(
				[setupTx.splitCoins(setupTx.gas, [2_000_000_000n])],
				coinsOnlyKeypair.toHaneulAddress(),
			);
			setupTx.transferObjects(
				[setupTx.splitCoins(setupTx.gas, [2_000_000_000n])],
				coinsAndBalanceKeypair.toHaneulAddress(),
			);
			setupTx.moveCall({
				target: `${packageId}::test::mint`,
				arguments: [
					...
					setupTx.pure.u64(50),
					setupTx.pure.address(coinsOnlyKeypair.toHaneulAddress()),
				],
			});
			setupTx.moveCall({
				target: `${packageId}::test::mint`,
				arguments: [
					setupTx.object(treasuryCapId!),
					setupTx.pure.u64(50),
					setupTx.pure.address(coinsAndBalanceKeypair.toHaneulAddress()),
				],
			});
			...
			const coins = await publishToolbox.grpcClient.core.listCoins({
				owner: coinsAndBalanceKeypair.toHaneulAddress(),
				coinType: testType,
			});
			const depositTx = new Transaction();
			const [coinToDeposit] = depositTx.splitCoins(depositTx.object(coins.objects[0].objectId), [
			...
			depositTx.moveCall({
				...
				typeArguments: [testType],
				arguments: [balToDeposit, depositTx.pure.address(coinsAndBalanceKeypair.toHaneulAddress())],
			});
		...
		testWithAllClients('balance::zero — createBalance with zero amount', async (client) => {
			...
			tx.setSender(coinsOnlyKeypair.toHaneulAddress());
			...
			expect(resolved.commands).toEqual([
				{
					MoveCall: {
						package: normalizeHaneulAddress('0x2'),
						module: 'balance',
						function: 'zero',
						typeArguments: [testTypeZero],
						arguments: [],
		...
		testWithAllClients(
			...
			async (client) => {
				...
				tx.transferObjects([coin], new Ed25519Keypair().toHaneulAddress());
				tx.setSender(coinsAndBalanceKeypair.toHaneulAddress());
		...
		// --- Path 1: Direct Withdrawal with HANEUL/gas balance ---
		testWithAllClients(
			'Path 1 — createBalance with HANEUL address balance (direct withdrawal)',
			async (client) => {
				// Use coinsOnlyKeypair which has HANEUL coins — deposit some to address balance
				const depositTx = new Transaction();
				const [coinToDeposit] = depositTx.splitCoins(depositTx.gas, [100_000_000n]);
				depositTx.moveCall({
					target: '0x2::coin::send_funds',
					typeArguments: ['0x2::haneul::HANEUL'],
					arguments: [coinToDeposit, depositTx.pure.address(coinsOnlyKeypair.toHaneulAddress())],
				});
				const depositResult = await client.core.signAndExecuteTransaction({
					transaction: depositTx,
		...
		testWithAllClients(
			...
			async (client) => {
				...
				tx.transferObjects([coin1, coin2], new Ed25519Keypair().toHaneulAddress());
				tx.setSender(coinsOnlyKeypair.toHaneulAddress());
		...
		testWithAllClients(
			'Path 2 — createBalance with HANEUL/gas (GasCoin, no remainder)',
			async (client) => {
		...
		testWithAllClients(
			...
			async (client) => {
				...
				tx.transferObjects([coinResult, coinFromBal], new Ed25519Keypair().toHaneulAddress());
				tx.setSender(coinsOnlyKeypair.toHaneulAddress());
	...
	describe('coin reservation (JSON-RPC only)', () => {
		...
		it('tx.coin with gas type uses coin reservation ref when AB is insufficient', async () => {
			...
			// Deposit HANEUL into address balance via send_funds
			const depositTx = new Transaction();
			const [coinToDeposit] = depositTx.splitCoins(depositTx.gas, [depositAmount]);
			depositTx.moveCall({
				target: '0x2::coin::send_funds',
				typeArguments: ['0x2::haneul::HANEUL'],
				arguments: [coinToDeposit, depositTx.pure.address(signer.toHaneulAddress())],
			});
			const depositResult = await client.core.signAndExecuteTransaction({
				transaction: depositTx,
				signer: signer,
		...
		it('tx.coin with gas type combines coins + address balance when neither alone is sufficient', async () => {
			...
			// Fund signer with a HANEUL coin
			const fundTx = new Transaction();
			fundTx.transferObjects(
				[fundTx.splitCoins(fundTx.gas, [2_000_000_000n])],
				signer.toHaneulAddress(),
		...
		it('tx.balance with gas type executes via address balance withdrawal', async () => {
			...
			// Use tx.balance instead of tx.coin — produces Balance<HANEUL>
			...
			tx.transferObjects([coinFromBal], new Ed25519Keypair().toHaneulAddress());
			tx.setSender(signer.toHaneulAddress());
