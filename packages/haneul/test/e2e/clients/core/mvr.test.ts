...
import { HaneulJsonRpcClient, JsonRpcHTTPTransport } from '../../../../src/jsonRpc/index.js';
...
describe('Core API - MVR (Move Registry)', () => {
	...
	beforeAll(async () => {
		...
		const testnetJsonRpcUrl = 'https://fullnode.testnet.haneul.io';
		const testnetGrpcUrl = 'https://fullnode.testnet.haneul.io:443';

		jsonRpcClient = new HaneulJsonRpcClient({
			network: 'testnet',
			transport: new JsonRpcHTTPTransport({
				url: testnetJsonRpcUrl,
			}),
		});
		grpcClient = new HaneulGrpcClient({
			network: 'testnet',
			baseUrl: testnetGrpcUrl,
		});
		graphqlClient = new HaneulGraphQLClient({
			network: 'testnet',
	...
	describe('MVR Type Resolution', () => {
		...
		testWithAllClients('should resolve type without MVR name (passthrough)', async (client) => {
			...
			const typeString = `0x2::coin::Coin<0x2::haneul::HANEUL>`;
