...
describe('Core API - Coin Metadata', () => {
	...
	describe('getCoinMetadata', () => {
		it('all clients return same data for HANEUL metadata', async () => {
			await toolbox.expectAllClientsReturnSameData((client) =>
				client.core.getCoinMetadata({ coinType: HANEUL_TYPE_ARG }),
			);
		});
		...
		testWithAllClients('should get metadata for HANEUL coin', async (client) => {
			...
			expect(result.coinMetadata!.name).toBe('Haneul');
			expect(result.coinMetadata!.symbol).toBe('HANEUL');
