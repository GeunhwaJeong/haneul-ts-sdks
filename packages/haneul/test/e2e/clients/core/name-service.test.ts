...
describe('Core API - Name Service', () => {
	...
	describe('defaultNameServiceName', () => {
		// Note: HaneulNS is not configured on localnet, so we expect specific behavior
		// gRPC throws "HaneulNS not configured for this network"
		...
		testWithAllClients('should handle name service request', async (client) => {
			try {
				...
				// On localnet without HaneulNS, name will be null or undefined
				if (result.data.name !== null && result.data.name !== undefined) {
					expect(typeof result.data.name).toBe('string');
				}
			} catch (error) {
				// gRPC and GraphQL may throw errors when HaneulNS is not configured
