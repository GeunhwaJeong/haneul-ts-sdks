		...
		testWithAllClients(
			...
			async (client) => {
				...
				const bear = objects.find(
					(o) => !(o instanceof Error) && o.objectId === normalizeHaneulAddress(demoBearId),
				);
				const simple = objects.find(
					(o) => !(o instanceof Error) && o.objectId === normalizeHaneulAddress(simpleObjectId),
				);
	...
	describe('display v1 → v2 migration', () => {
		...
		beforeAll(async () => {
			...
			setupTx.transferObjects(
				[cap],
				setupTx.pure.address(publisherKeypair!.getPublicKey().toHaneulAddress()),
			);
