...
describe('Core API - Coins', () => {
	...
	describe('getCoins', () => {
		...
		testWithAllClients('should get coins for an address', async (client) => {
			...
			for (const coin of result.objects) {
				...
				expect(coin.type).toContain('haneul::HANEUL');
		...
		testWithAllClients('should return empty for address with no coins', async (client) => {
			...
			const emptyAddress = new Ed25519Keypair().toHaneulAddress();
			const result = await client.core.listCoins({
				owner: emptyAddress,
				coinType: HANEUL_TYPE_ARG,
			});
	...
	describe('listCoins pagination limit', () => {
		...
		testWithAllClients('should respect the limit and paginate', async (client) => {
			...
			// The address has exactly 3 HANEUL coins, so a limit of 2 must return exactly 2
	...
	describe('getBalance', () => {
		it('all clients return same data: getBalance', async () => {
			await toolbox.expectAllClientsReturnSameData((client) =>
				client.core.getBalance({ owner: testAddress, coinType: HANEUL_TYPE_ARG }),
			);
		});
		...
		testWithAllClients('should get balance for an address', async (client) => {
			...
			expect(result.balance.coinType).toContain('::haneul::HANEUL');
	...
	describe('getAllBalances', () => {
		...
		// Skipped for JSON-RPC: its `haneulx_getAllBalances` endpoint takes no pagination
