...
import { HaneulGrpcClient, GrpcWebFetchTransport } from '../../../../src/grpc/index.js';
...
/**
 * E2E tests verifying that HaneulGrpcClient works correctly with both the default
...
describe('gRPC transport variants', () => {
	...
	let nativeGrpcClient: HaneulGrpcClient | undefined;
	let grpcWebCustomClient: HaneulGrpcClient;
	...
	beforeAll(async () => {
		...
		if (GrpcTransport && ChannelCredentials) {
			...
			nativeGrpcClient = new HaneulGrpcClient({
				network: 'localnet',
		...
		grpcWebCustomClient = new HaneulGrpcClient({
			network: 'localnet',
	...
	describe('getBalance', () => {
		it.skipIf(!GrpcTransport)('native gRPC transport returns balance', async () => {
			...
			expect(balance.coinType).toContain('haneul::HANEUL');
