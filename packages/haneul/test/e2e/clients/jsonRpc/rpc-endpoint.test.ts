...
import { HaneulGasData } from '../../../../src/jsonRpc/index.js';
...
describe('Invoke any RPC endpoint', () => {
	...
	it('haneulx_getOwnedObjects', async () => {
		...
		const gasObjects = await toolbox.jsonRpcClient.call<{ data: HaneulGasData }>(
	...
	it('haneul_getObjectOwnedByAddress Error', async () => {
		await expect(toolbox.jsonRpcClient.call('haneulx_getOwnedObjects', [])).rejects.toThrowError();
	});

	it('haneulx_getCommitteeInfo', async () => {
		const committeeInfoExpected = await toolbox.jsonRpcClient.getCommitteeInfo();

		const committeeInfo = await toolbox.jsonRpcClient.call('haneulx_getCommitteeInfo', []);
