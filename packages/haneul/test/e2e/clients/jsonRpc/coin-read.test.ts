...
// See: haneul/crates/haneul-types/src/coin_reservation.rs
...
describe('CoinRead API', () => {
	...
	it('Get coins with/without type', async () => {
		const haneulCoins = await toolbox.jsonRpcClient.getCoins({
		...
		const someHaneulCoins = await toolbox.jsonRpcClient.getCoins({
			owner: toolbox.address(),
			limit: 3,
		});
	...
	it('Get balance with/without type', async () => {
		const haneulBalance = await toolbox.jsonRpcClient.getBalance({
			owner: toolbox.address(),
		});
	...
	describe('Address Balance', () => {
		it('filters out fake address balance coins from listCoins', async () => {
			// Deposit some HANEUL to the address balance
