...
import { ClientWithCoreApi, HaneulClientTypes } from '../../src/client/index.js';

function getOwnerAddress(o: HaneulClientTypes.ChangedObject): string | undefined {
	return (
...
describe('Transfer to Object', () => {
	...
	let parentObjectId: HaneulClientTypes.ChangedObject;
	let receiveObjectId: HaneulClientTypes.ChangedObject;
	...
	beforeEach(async () => {
		...
		const y = x.Transaction?.effects?.changedObjects
			.filter((o: HaneulClientTypes.ChangedObject) => o.idOperation === 'Created')
			.map((o: HaneulClientTypes.ChangedObject) => getOwnerAddress(o))!;
		receiveObjectId = x.Transaction?.effects?.changedObjects!.filter(
			(o: HaneulClientTypes.ChangedObject) =>
		...
		const sharedObject = x.Transaction?.effects?.changedObjects!.filter(
			(o: HaneulClientTypes.ChangedObject) => getOwnerAddress(o) === undefined,
		)[0]!;
