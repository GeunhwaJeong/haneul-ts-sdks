...
import type { HaneulObjectChangePublished } from '../../../src/jsonRpc/index.js';
import {
	getJsonRpcFullnodeUrl,
	HaneulJsonRpcClient,
	JsonRpcHTTPTransport,
} from '../../../src/jsonRpc/index.js';
import { HaneulGrpcClient } from '../../../src/grpc/index.js';
import { HaneulGraphQLClient } from '../../../src/graphql/index.js';
import type { Keypair } from '../../../src/cryptography/index.js';
import {
	FaucetRateLimitError,
	getFaucetHost,
	requestHaneulFromFaucetV2,
...
import { HANEUL_TYPE_ARG, normalizeHaneulAddress } from '../../../src/utils/index.js';
import type { ClientWithCoreApi } from '../../../src/client/core.js';
import type { HaneulClientTypes } from '../../../src/client/types.js';
...
export class TestToolbox {
	...
	constructor(keypair: Ed25519Keypair, url: string = DEFAULT_FULLNODE_URL, configPath: string) {
		...
		this.jsonRpcClient = new HaneulJsonRpcClient({
			network: 'localnet',
			transport: new JsonRpcHTTPTransport({
				url,
			}),
		});
		this.grpcClient = new HaneulGrpcClient({
			network: 'localnet',
			baseUrl: url,
		});
		// GraphQL port is injected by vitest setup
		const graphqlPort = inject('graphqlPort');
		this.graphqlClient = new HaneulGraphQLClient({
			network: 'localnet',
	...
	public async getActiveValidators() {
		return (await this.jsonRpcClient.getLatestHaneulSystemState()).activeValidators;
	}
	...
	async signAndExecuteTransaction<Include extends HaneulClientTypes.TransactionInclude = {}>(
		options: HaneulClientTypes.SignAndExecuteTransactionOptions<Include>,
	) {
	...
	async getSigner(config: SignerConfig): Promise<{ keypair: Ed25519Keypair; address: string }> {
		return this.#funderQueue.runTask(async () => {
			...
			await requestAndWaitForFaucet(
				this.#funderKeypair.getPublicKey().toHaneulAddress(),
				this.grpcClient,
				totalNeeded,
			);
...
export function getClient(url = DEFAULT_FULLNODE_URL): HaneulJsonRpcClient {
...
async function requestAndWaitForFaucet(
	...
	const request = async () => {
		const response = await retry(
			async () => await requestHaneulFromFaucetV2({ host: DEFAULT_FAUCET_URL, recipient: address }),
			{
				backoff: 'EXPONENTIAL',
				timeout: 1000 * 60,
...
export async function publishPackage(packageName: string, toolbox?: TestToolbox) {
	...
	return await retry(
		async () => {
			...
			const packageId = normalizeHaneulAddress(
				((publishTxn.objectChanges?.filter(
					(a) => a.type === 'published',
				) as HaneulObjectChangePublished[]) ?? [])[0]?.packageId,
			);
...
export async function upgradePackage(
	...
	const { stdout } = await execHaneulTools([
		'haneul',
		'move',
		'--client.config',
		toolbox.configPath,
...
/**
 * Run a `haneul keytool` command with an isolated keystore to avoid concurrent write conflicts.
 * Retries on failure since concurrent keytool calls can cause panics in the haneul binary.
...
export async function execKeytool(
	...
	for (let attempt = 0; attempt < maxRetries; attempt++) {
		...
		await execHaneulTools(['bash', '-c', `echo '[]' > ${keystorePath}`]);
		const result = await execHaneulTools([
			'haneul',
			'keytool',
			'--keystore-path',
...
/**
 * Parse JSON output from `haneul keytool --json` commands.
