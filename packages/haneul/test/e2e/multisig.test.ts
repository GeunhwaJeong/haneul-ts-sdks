...
import { decodeHaneulPrivateKey } from '../../src/cryptography/index.js';
...
describe('MultiSig with zklogin signature', () => {
	it('Execute tx with multisig with 1 sig and 1 zkLogin sig combined', async () => {
		...
		const epoch = await tempToolbox.jsonRpcClient.getLatestHaneulSystemState();
		const currentEpoch = Number(epoch.epoch);
		const maxEpoch = currentEpoch + 10;

		// Generate a zkLogin signature dynamically using haneul keytool
		...
		const multisigAddr = multiSigPublicKey.toHaneulAddress();
		...
		// This is the default ephemeral keypair used by the haneul keytool
		const parsed = decodeHaneulPrivateKey(
