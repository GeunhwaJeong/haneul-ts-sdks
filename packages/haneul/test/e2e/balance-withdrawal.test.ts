...
describe('Balance Withdrawal', () => {
	...
	it('deposits HANEUL to address balance and withdraws it', async () => {
		const depositAmount = 200_000_000n; // 0.2 HANEUL
		...
		const withdrawalAmount = 100_000_000n; // 0.1 HANEUL — less than deposit to leave room for gas
	...
	it('uses address balance for gas payment and withdrawal', async () => {
		const depositAmount = 200_000_000n; // 0.2 HANEUL (enough for gas + withdrawal)
		const withdrawalAmount = 50_000_000n; // 0.05 HANEUL

		// Step 1: Deposit HANEUL to address balance
		const depositTx = new Transaction();
		const [coinToDeposit] = depositTx.splitCoins(depositTx.gas, [depositAmount]);
		...
		const [genesisCheckpoint, gasPrice, systemState] = await Promise.all([
			...
			toolbox.jsonRpcClient.getLatestHaneulSystemState(),
		]);
		...
		tx.setGasBudget(100_000_000n); // 0.1 HANEUL budget
		tx.setExpiration({
			ValidDuring: {
