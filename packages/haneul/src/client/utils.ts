...
function parseBcsExecutionError(failure: {
	...
	switch (error.$kind) {
		...
		case 'HaneulMoveVerificationError':
		...
		case 'HaneulMoveVerificationTimedout':
...
): HaneulClientTypes.TransactionData {
	return (
		onlyTransactionKind
...
export function parseTransactionEffectsBcs(effects: Uint8Array): HaneulClientTypes.TransactionEffects {
...
}): HaneulClientTypes.TransactionEffects {
...
function parseTransactionEffectsV2({
	...
	const changedObjects = effects.changedObjects.map(
		([id, change]): HaneulClientTypes.ChangedObject => {
			return {
				objectId: id,
	...
	return {
		...
		unchangedConsensusObjects: effects.unchangedConsensusObjects.map(
			([objectId, object]): HaneulClientTypes.UnchangedConsensusObject => {
				return {
					kind: object.$kind,
					objectId: objectId,
					version:
