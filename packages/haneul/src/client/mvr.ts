...
export class MvrClient implements HaneulClientTypes.MvrMethods {
	#cache: ClientCache;
	#url?: string;
	#pageSize: number;
	#overrides: {
	...
	async resolvePackage({
		package: name,
	}: HaneulClientTypes.MvrResolvePackageOptions): Promise<HaneulClientTypes.MvrResolvePackageResponse> {
		if (!hasMvrName(name)) {
			return {
				package: name,
			};
	...
	async resolveType({
		type,
	}: HaneulClientTypes.MvrResolveTypeOptions): Promise<HaneulClientTypes.MvrResolveTypeResponse> {
		if (!hasMvrName(type)) {
			return {
				type,
			};
	...
	async resolve({
		types = [],
		packages = [],
	}: HaneulClientTypes.MvrResolveOptions): Promise<HaneulClientTypes.MvrResolveResponse> {
...
export function replaceNames(
	...
	resolved: HaneulClientTypes.MvrResolveResponse,
) {
	for (const command of builder.commands) {
