	...
	async getMoveFunction(
		...
		return {
			function: {
				...
				parameters:
					moveFunction.parameters?.map((param) => parseNormalizedHaneulMoveType(param.signature)) ??
					[],
				returns:
					moveFunction.return?.map(({ signature }) => parseNormalizedHaneulMoveType(signature)) ?? [],
			},
		};
	}
...
function mapOwner(owner: Object_Owner_FieldsFragment): HaneulClientTypes.ObjectOwner {
...
function parseTransaction<Include extends HaneulClientTypes.TransactionInclude = object>(
	...
	let balanceChanges: HaneulClientTypes.BalanceChange[] | undefined;
	...
	const status: HaneulClientTypes.ExecutionStatus =
	...
	const result: HaneulClientTypes.Transaction<Include> = {
		...
		balanceChanges: balanceChanges as HaneulClientTypes.Transaction<Include>['balanceChanges'],
...
function parseNormalizedHaneulMoveType(type: OpenMoveTypeSignature): HaneulClientTypes.OpenSignature {
...
): HaneulClientTypes.ExecutionError {
...
): HaneulClientTypes.MoveLocation | undefined {
...
function parseGraphQLCleverError(
	executionError: GraphQLExecutionError,
): HaneulClientTypes.CleverError | undefined {
	...
	if ('datatype' in type) {
		return {
			...
			datatype: {
				...
				typeParameters: type.datatype.typeParameters.map((t) => parseNormalizedHaneulMoveTypeBody(t)),
			},
		};
	}
