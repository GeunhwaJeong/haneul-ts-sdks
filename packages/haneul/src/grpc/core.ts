...
} from './proto/haneul/rpc/v2/execution_status.js';
import {
	...
	async getMoveFunction(
		...
		return {
			function: {
				...
				parameters:
					response.function?.parameters?.map((param) => parseNormalizedHaneulMoveType(param)) ?? [],
				returns: response.function?.returns?.map((ret) => parseNormalizedHaneulMoveType(ret)) ?? [],
			},
		};
	}
...
function parseGrpcExecutionError(error: GrpcExecutionError): HaneulClientTypes.ExecutionError {
...
function parseMoveAbort(abort: GrpcMoveAbort): HaneulClientTypes.MoveAbort {
	return {
		abortCode: String(abort.abortCode ?? 0n),
		location: { ...abort.location },
...
): null | HaneulClientTypes.UnchangedConsensusObject['kind'] {
	if (kind == null) {
		return null;
	}
	switch (kind) {
...
export function parseTransactionEffects({
...
}): HaneulClientTypes.TransactionEffects | null {
	if (!effects) {
		return null;
	}

	const changedObjects = effects.changedObjects.map((change): HaneulClientTypes.ChangedObject => {
		return {
	...
	return {
		...
		unchangedConsensusObjects: effects.unchangedConsensusObjects.map(
			(object): HaneulClientTypes.UnchangedConsensusObject => {
				return {
...
function parseTransaction<Include extends HaneulClientTypes.TransactionInclude = object>(
	...
	const status: HaneulClientTypes.ExecutionStatus = transaction.effects?.status?.success
...
function parseNormalizedHaneulMoveType(type: OpenSignature): HaneulClientTypes.OpenSignature {
	...
	return {
		reference,
		body: parseNormalizedHaneulMoveTypeBody(type.body!),
	};
}

function parseNormalizedHaneulMoveTypeBody(type: OpenSignatureBody): HaneulClientTypes.OpenSignatureBody {
	switch (type.type) {
		...
		case OpenSignatureBody_Type.VECTOR:
			return {
				$kind: 'vector',
				vector: parseNormalizedHaneulMoveTypeBody(type.typeParameterInstantiation[0]),
			};
			...
			return {
				...
				datatype: {
					...
					typeParameters: type.typeParameterInstantiation.map((t) =>
						parseNormalizedHaneulMoveTypeBody(t),
					),
				},
			};
