...
export interface CreateSponsorOptions<T extends readonly ValidateItem[] = readonly ValidateItem[]> {
	...
	signer: Signer;
	/** Haneul client used to build, simulate, and analyze transactions. */
	client: ClientWithCoreApi;
	/**
...
export type SignAndExecuteResult<
	Include extends HaneulClientTypes.TransactionInclude = { effects: true },
> = SponsorRejection | HaneulClientTypes.TransactionResult<Include>;
 ...
 * {@link HaneulClientTypes.ExecuteTransactionOptions}; every other core-execute prop
...
export type SignAndExecuteOptions<
	Include extends Omit<HaneulClientTypes.TransactionInclude, 'effects'> = {},
> = Omit<HaneulClientTypes.ExecuteTransactionOptions, 'transaction' | 'signatures' | 'include'> & {
...
export function assertSponsorable(transaction: Transaction, sponsor: string): void {
	...
	if (!owner || normalizeHaneulAddress(owner) !== sponsor) {
...
export class Sponsor<TOptions extends object = object> {
	...
	get address(): string {
		return normalizeHaneulAddress(this.#signer.toHaneulAddress());
	}
