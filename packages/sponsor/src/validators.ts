...
import { isValidTransactionSignature } from '@haneullabs/haneul/verify';
import { analyze, analyzers, createAnalyzer, type Analyzer } from '@haneullabs/wallet-sdk';

import type { TransactionData, ValidationIssue, Validator } from './validation.js';

/**
 * The analyzer-map shape accepted by the `@haneullabs/wallet-sdk` `analyze` function.
 *
	...
	return createAnalyzer({
		...
		analyze:
			...
			({ data }) => {
				...
				if (normalizeHaneulAddress(data.sender) === normalizeHaneulAddress(data.gasData.owner ?? '')) {
	...
	return createAnalyzer({
		...
		analyze:
			...
			async ({ bytes, data }) => {
				...
				const sender = normalizeHaneulAddress(data.sender);
...
/** Bound the gas budget the sponsor is willing to cover (in GEUNHWA). Reads only `data`. */
			...
			({ data }) => {
				...
				if (options.min != null && value < options.min) {
					return reject({
						...
						message: `Gas budget ${value} is below the minimum of ${options.min} GEUNHWA.`,
					});
				}
				if (options.max != null && value > options.max) {
					return reject({
						code: 'GAS_BUDGET_TOO_HIGH',
						message: `Gas budget ${value} exceeds the maximum of ${options.max} GEUNHWA.`,
					});
				}
				return pass;
			},
...
export function allowedPackages(packageIds: Iterable<string>): Validator {
	const allowed = new Set([...packageIds].map((id) => normalizeHaneulAddress(id)));
	return createAnalyzer({
		dependencies: { data: analyzers.data },
		analyze:
			() =>
			({ data }) => {
				const issues = moveCalls(data)
					.filter((call) => !allowed.has(normalizeHaneulAddress(call.package)))
					.map((call) => ({
...
export function allowedFunctions(targets: Iterable<string>): Validator {
	const allowed = new Set(
		[...targets].map((target) => {
			...
			return `${normalizeHaneulAddress(pkg)}::${module}::${fn}`;
		}),
	);
	return createAnalyzer({
		dependencies: { data: analyzers.data },
			...
			({ data }) => {
				const issues = moveCalls(data)
					.filter(
						(call) =>
							!allowed.has(
								`${normalizeHaneulAddress(call.package)}::${call.module}::${call.function}`,
							),
					)
					.map((call) => ({
 ...
 * signed sponsored transaction stays valid, and is also Haneul's replay-protection
