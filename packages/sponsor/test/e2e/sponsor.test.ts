...
/**
 * A realistic sponsored transaction: the sender spends 1 GEUNHWA of its *own* HANEUL
...
function userCommands() {
	...
	tx.transferObjects([coin], sponsorKey.toHaneulAddress());
	tx.setSender(senderKey.toHaneulAddress());
	return tx;
}
...
describe(`Sponsor e2e (${NETWORK})`, () => {
	...
	it('runs a custom balanceFlows validator (rejects when underpaid)', async (ctx) => {
		...
		// Require the sponsor net at least 1 HANEUL; the realistic tx pays 1 GEUNHWA → reject.
