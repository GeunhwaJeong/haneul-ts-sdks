...
import { Transaction, TransactionDataBuilder } from '@haneullabs/haneul/transactions';
import { normalizeHaneulAddress, toBase58, toBase64 } from '@haneullabs/haneul/utils';
...
describe('Sponsor.signTransaction', () => {
	it('validates and produces a verifiable sponsor signature', async () => {
		...
		const sender = new Ed25519Keypair().toHaneulAddress();
		const tx = resolvedTransaction({ sender, sponsor: sponsorKey.toHaneulAddress() });
		...
		expect(normalizeHaneulAddress(pk.toHaneulAddress())).toBe(sponsor.address);
	});
	...
	it('co-signs a user-signed transaction and returns both signatures', async () => {
		...
		const tx = resolvedTransaction({
			sender: senderKey.toHaneulAddress(),
			sponsor: sponsorKey.toHaneulAddress(),
		});
	...
	it('returns a Rejected result when sender equals sponsor', async () => {
		const sponsorKey = new Ed25519Keypair();
		const tx = resolvedTransaction({
			sender: sponsorKey.toHaneulAddress(),
			sponsor: sponsorKey.toHaneulAddress(),
		});
...
describe('Sponsor.signTransaction — analyzer behavior', () => {
	function txFor(sponsorKey: Ed25519Keypair) {
		return resolvedTransaction({
			sender: new Ed25519Keypair().toHaneulAddress(),
			sponsor: sponsorKey.toHaneulAddress(),
		});
	}
...
describe('Sponsor.signAndExecuteTransaction', () => {
	it('returns custom result fields (events) requested via `include`, with effects forced on', async () => {
		...
		const events: HaneulClientTypes.Event[] = [
			{
				packageId: normalizeHaneulAddress('0x2'),
				module: 'foo',
				sender: senderKey.toHaneulAddress(),
		...
		if (result.$kind === 'Transaction') {
			...
			const got: HaneulClientTypes.Event[] = result.Transaction.events;
...
describe('offline-only validation (no client calls)', () => {
	const sponsorKey = new Ed25519Keypair();
	const sponsorAddr = normalizeHaneulAddress(sponsorKey.toHaneulAddress());
	const sender = new Ed25519Keypair().toHaneulAddress();
...
describe('assertSponsorable', () => {
	const sponsor = normalizeHaneulAddress('0x5');
	...
	const resolvedTx = () => {
		const tx = new Transaction();
		tx.setSender(normalizeHaneulAddress('0xa'));
		tx.setGasOwner(sponsor);
	...
	it('rejects a gas owner that is not the sponsor', () => {
		const tx = resolvedTx();
		tx.setGasOwner(normalizeHaneulAddress('0x9'));
