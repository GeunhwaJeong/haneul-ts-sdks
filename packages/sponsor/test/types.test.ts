...
const needsBalanceChanges = createAnalyzer({
	...
	analyze:
		...
		({ transactionResponse }) => {
			...
			const changes: HaneulClientTypes.BalanceChange[] = transactionResponse.balanceChanges;
...
const needsEvents = createAnalyzer({
	...
	analyze:
		...
		({ transactionResponse }) => {
			...
			const events: HaneulClientTypes.Event[] = transactionResponse.events;
...
async function _executeOptions() {
	...
	if (res.$kind === 'Transaction') {
		const _effects: HaneulClientTypes.TransactionEffects = res.Transaction.effects;
		const _events: HaneulClientTypes.Event[] = res.Transaction.events;
		void _effects;
		void _events;
	}
	...
	if (plain.$kind === 'Transaction') {
		const _effects: HaneulClientTypes.TransactionEffects = plain.Transaction.effects;
		void _effects;
	}
