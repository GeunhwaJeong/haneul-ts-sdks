...
Primitives for building gas-sponsorship flows on Haneul. You are the **sponsor operator**: you bring a
...
the [`@haneullabs/wallet-sdk` transaction analyzer](https://www.npmjs.com/package/@haneullabs/wallet-sdk).
...
at [sdk.haneul-labs.com/sponsor](https://sdk.haneul-labs.com/sponsor).
...
```sh
npm install @haneullabs/sponsor @haneullabs/haneul
```
...
import { createSponsor, defaults, gasBudget, allowedPackages } from '@haneullabs/sponsor';
...
draws from the sponsor's on-chain HANEUL balance rather than from specific gas coins. This has two
benefits:
...
You fund the address balance by depositing HANEUL into it (e.g. a one-time
...
`gasCoinNotUsed()` (a default) enforces this — when the user needs HANEUL, they spend their own with
...
HANEUL, source it from their own balance with `useGasCoin: false`:
...
```ts
import { analyzers, createAnalyzer } from '@haneullabs/sponsor';
...
package adds (see [`@haneullabs/wallet-sdk`](https://www.npmjs.com/package/@haneullabs/wallet-sdk) for the
...
// Loads the *sponsor's* HANEUL balance via options.client (the gas owner is the sponsor)…
	...
	analyze:
		...
		({ sponsorBalance }) =>
			sponsorBalance >= 1_000_000_000n
				? { result: null }
				: { result: [{ code: 'RESERVE_LOW', message: 'Sponsor reserve below 1 HANEUL.' }] },
});
...
‡ Verifies (via `@haneullabs/haneul`'s `isValidTransactionSignature`) that **every** supplied user
