...
import { deriveObjectID, normalizeHaneulAddress } from '@haneullabs/haneul/utils';
...
const PAS_PACKAGE = normalizeHaneulAddress('0xbad1');
const PAS_NAMESPACE = normalizeHaneulAddress('0xb0b5');
...
const ACCOUNT_A_ID = normalizeHaneulAddress('0xacc1');
const ACCOUNT_B_ID = normalizeHaneulAddress('0xacc2');
const POLICY = normalizeHaneulAddress('0xb01cabe');

function deriveTestAccountAddress(owner: string): string {
	const key = bcs.Address.serialize(normalizeHaneulAddress(owner)).toBytes();
	return deriveObjectID(PAS_NAMESPACE, `${PAS_PACKAGE}::keys::AccountKey`, key);
}

function setupClient(accountIds: string[]): MockHaneulClient {
	...
	client.addMoveFunction({
		packageId: normalizeHaneulAddress('0x9999'),
		moduleName: 'issuer',
...
async function analyzeTx(client: MockHaneulClient, tx: Transaction) {
	return analyze(
		{ balanceFlows },
		{
			client,
...
describe('PAS balanceFlows handler', () => {
	it('credits deposit_balance to the account address', async () => {
		...
		expect(
			deltas?.[normalizeHaneulAddress(DEFAULT_SENDER)]?.find((f) => f.coinType === USDC)?.amount,
		).toBe(-500_000_000n);
	...
	it('unsafe_send_balance credits the recipients derived account address', async () => {
		const client = setupClient([ACCOUNT_A_ID]);
		const recipientWallet = normalizeHaneulAddress('0xbeef');
	...
	it('resolves Account args that come from a prior account::create Result', async () => {
		const client = setupClient([ACCOUNT_A_ID]);
		const freshOwner = normalizeHaneulAddress('0xfa7e');
	...
	it('does nothing for MoveCalls in other packages', async () => {
		...
		tx.moveCall({
			target: `${normalizeHaneulAddress('0x9999')}::issuer::approve_transfer`,
			arguments: [tx.object(ACCOUNT_A_ID), tx.object(POLICY)],
		});
		tx.transferObjects([split], tx.pure.address(normalizeHaneulAddress('0x456')));
	...
	it('handles mixed Object/Result account args on send_balance', async () => {
		const client = setupClient([ACCOUNT_A_ID]);
		const freshOwner = normalizeHaneulAddress('0xca11');
