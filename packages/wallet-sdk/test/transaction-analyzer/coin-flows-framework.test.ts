 ...
 * analyzer sees a real sponsored transaction. Add a sponsor-owned HANEUL coin
 * to the client for gas payment. Returns the patched JSON string.
 */
function sponsorize(client: MockHaneulClient, txJson: string, sponsor: string = SPONSOR): string {
...
describe('Coin Flows - Framework MoveCall Tests', () => {
	...
	it('handles coin::send_funds to sender (no outflow)', async () => {
		...
		expect(haneulFlow?.amount).toBe(10000000n); // gas budget only
	});
	...
	it('handles coin::send_funds to other address (outflow)', async () => {
		...
		const haneulFlow = results.coinFlows.result?.outflows.find((f) => f.coinType === HANEUL);
		expect(haneulFlow?.amount).toBe(210000000n); // 200M + gas budget
	});
	...
	it('handles coin::into_balance (value preserved, not double-counted)', async () => {
		...
		expect(haneulFlow?.amount).toBe(310000000n);
	});
	...
	it('handles coin::divide_into_n (assumes full consumption)', async () => {
		...
		const haneulFlow = results.coinFlows.result?.outflows.find((f) => f.coinType === HANEUL);
		expect(haneulFlow?.amount).toBe(5010000000n); // full coin + gas budget
	});
	...
	it('handles coin::join via MoveCall', async () => {
		...
		const haneulFlow = results.coinFlows.result?.outflows.find((f) => f.coinType === HANEUL);
		expect(haneulFlow?.amount).toBe(7510000000n); // 5 + 2.5 HANEUL + gas
	});
...
describe('Coin Flows - Combination Chain Tests', () => {
	...
	it('mixed coin types with AB withdrawal', async () => {
		...
		// HANEUL from gas coin, transfer to self
		const [haneulSplit] = tx.splitCoins(tx.gas, [50000000n]);
		tx.transferObjects([haneulSplit], tx.pure.address(DEFAULT_SENDER));
		...
		const haneulFlow = results.coinFlows.result?.outflows.find((f) => f.coinType === HANEUL);
		expect(haneulFlow?.amount).toBe(10000000n); // gas budget only, HANEUL returned to self
	});
});
...
describe('Coin Flows - Per-party tracking', () => {
	...
	it('sponsored gas is attributed to sponsor, not sender', async () => {
		...
		expect(results.coinFlows.result?.outflows.find((f) => f.coinType === HANEUL)).toBeUndefined();

		const sponsorHaneul = results.balanceFlows.result?.sponsor?.find((f) => f.coinType === HANEUL);
		expect(sponsorHaneul?.amount).toBe(-10000000n); // gas budget only
	});
...
describe('Coin Flows - Transfer to Self Bug Fix', () => {
	it('should not create outflow for non-HANEUL transfer to self', async () => {
		const client = new MockHaneulClient();
		const tx = new Transaction();
		tx.setSender(DEFAULT_SENDER);
		...
		// HANEUL should have gas budget outflow only
		const haneulFlow = results.coinFlows.result?.outflows.find((f) => f.coinType === HANEUL);
...
describe('balanceFlows - sponsor shape', () => {
	it('returns null sponsor flows for non-sponsored transactions', async () => {
		...
		const senderHaneul = results.balanceFlows.result?.sender.find((f) => f.coinType === HANEUL);
		expect(senderHaneul?.amount).toBeLessThan(0n);
	});
	...
	it('returns a non-null sponsor flow list for sponsored transactions (empty if sponsor had no movement)', async () => {
		...
		// Sponsor paid the gas budget → sponsor HANEUL delta is negative.
		const sponsorHaneul = results.balanceFlows.result?.sponsor?.find((f) => f.coinType === HANEUL);
		expect(sponsorHaneul?.amount).toBe(-10000000n);
	});
});
...
describe('balanceFlows - issues', () => {
	it('flags gas budget exceeding the total gas payment balance', async () => {
		...
		json.gasData.budget = '10000000'; // 0.01 HANEUL > 1 geunhwa balance
	...
	it('flags coin::split taking more than the source coin holds', async () => {
		...
		tx.moveCall({
			target: '0x2::coin::split',
			typeArguments: ['0x2::haneul::HANEUL'],
			arguments: [coin, tx.pure.u64(10_000_000_000n)], // 10 HANEUL > 5 HANEUL
		});
	...
	it('flags SplitCoins taking more than a non-AB coin holds', async () => {
		...
		const coin = tx.object(TEST_COIN_1_ID); // 5 HANEUL, owned coin (not AB-backed)
		const [split] = tx.splitCoins(coin, [10_000_000_000n]); // 10 HANEUL > 5 HANEUL
		tx.transferObjects([split], tx.pure.address('0x456'));
...
describe('balanceFlows - excludeGasBudget', () => {
	it('omits the gas budget from sender deltas when payment uses a coin', async () => {
		...
		// net sender HANEUL delta is zero.
		const haneul = results.balanceFlows.result?.sender.find((f) => f.coinType === HANEUL);
		expect(haneul?.amount ?? 0n).toBe(0n);
	});
	...
	it('still tracks non-gas outflows when excludeGasBudget is true', async () => {
		...
		const tx = new Transaction();
		tx.setSender(DEFAULT_SENDER);
		// Transfer 200M HANEUL from a real coin (not from gas): this outflow
		...
		const haneul = results.balanceFlows.result?.sender.find((f) => f.coinType === HANEUL);
		expect(haneul?.amount).toBe(-200_000_000n);
	});
	...
	it('defaults to including the gas budget', async () => {
		...
		const haneul = results.balanceFlows.result?.sender.find((f) => f.coinType === HANEUL);
		expect(haneul?.amount).toBe(-10_000_000n);
	});
});
