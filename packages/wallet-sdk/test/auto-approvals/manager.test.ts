...
const RAW_HANEUL = '0x2::haneul::HANEUL';
const HANEUL = normalizeHaneulAddress('0x2') + '::haneul::HANEUL';
...
function autoApprovalAnalysis(overrides: Partial<AutoApprovalAnalysis> = {}): AutoApprovalAnalysis {
	...
	const senderAddresses = overrides.senderAddresses ?? [normalizeHaneulAddress('0x1')];
...
describe('AutoApprovalManager', () => {
	test('analyzes bytes once with explicit operation type and shared balance flows', async () => {
		...
		expect(results.autoApproval.result?.senderBalanceFlows).toEqual([
			{ coinType: HANEUL, amount: -10_000_000n },
		]);
	});
	...
	test('adds configured sender addresses to the transaction sender', async () => {
		...
		const extraSender = normalizeHaneulAddress('0x456');
		const results = await analyze(
			{
				autoApproval: autoApprovalAnalyzer,
			},
	...
	test('matches unnormalized policy and budget coin types against analyzer balance flows', async () => {
		...
		expect(manager.getSettings()?.coinBudgets).toEqual({ [HANEUL]: '10000000' });
		...
		expect(manager.getSettings()?.coinBudgets).toEqual({ [HANEUL]: '0' });
	});
	...
	test('uses signed sender balance outflows for policy and budget checks', () => {
		...
		const sender = normalizeHaneulAddress('0x1');
		const analysis: AutoApprovalResult = {
			status: 'success',
			result: autoApprovalAnalysis({
				senderAddresses: [sender],
				senderBalanceFlows: [{ coinType: HANEUL, amount: -4n }],
				balanceFlows: {
					byAddress: { [sender]: [{ coinType: HANEUL, amount: -4n }] },
					sender: [{ coinType: HANEUL, amount: -4n }],
					sponsor: null,
				},
			}),
		};
		...
		expect(manager.getSettings()?.coinBudgets[HANEUL]).toBe('6');

		manager.revertTransaction(analysis);

		expect(manager.getSettings()?.coinBudgets[HANEUL]).toBe('10');
		expect(manager.getState().pendingDigests).toEqual([]);
	});
	...
	test('uses multiple sender addresses for policy and budget checks', () => {
		const manager = managerWithBalanceSettings();
		const sender = normalizeHaneulAddress('0x1');
		const extraSender = normalizeHaneulAddress('0x2');
		const analysis: AutoApprovalResult = {
			status: 'success',
			result: autoApprovalAnalysis({
				senderAddresses: [sender, extraSender],
				senderBalanceFlows: [{ coinType: HANEUL, amount: -6n }],
				balanceFlows: {
					byAddress: {
						[sender]: [{ coinType: HANEUL, amount: -4n }],
						[extraSender]: [{ coinType: HANEUL, amount: -2n }],
					},
					sender: [{ coinType: HANEUL, amount: -4n }],
					sponsor: null,
				},
		...
		expect(manager.getSettings()?.coinBudgets[HANEUL]).toBe('4');
	});

	test('gas-only HANEUL outflows are budgeted and reconciled from effects', () => {
		const manager = managerWithBalanceSettings();
		manager.updateSettings({
			approvedOperations: ['test-operation'],
			expiration: Date.now() + 1000 * 60 * 60,
		...
		const analysis: AutoApprovalResult = {
			...
			result: autoApprovalAnalysis({
				senderAddresses: [sender],
				senderBalanceFlows: [{ coinType: HANEUL, amount: -10_000_000n }],
				coinValues: { total: 0, coinTypesWithoutPrice: [], coinTypes: [] },
				balanceFlows: {
					byAddress: {
						[sender]: [{ coinType: HANEUL, amount: -10_000_000n }],
					},
					sender: [{ coinType: HANEUL, amount: -10_000_000n }],
					sponsor: null,
				},
		...
		manager.commitTransaction(analysis);
		expect(manager.getSettings()?.coinBudgets[HANEUL]).toBe('0');

		manager.applyTransactionEffects(analysis, {
			digest: 'test-digest',
			balanceChanges: [{ address: sender, coinType: HANEUL, amount: '-5000000' }],
		} as unknown as Parameters<AutoApprovalManager['applyTransactionEffects']>[1]);

		expect(manager.getSettings()?.coinBudgets[HANEUL]).toBe('5000000');
		expect(manager.getState().pendingDigests).toEqual([]);
	});
	...
	test('reconciles transaction effects with sender balance changes only', () => {
		...
		manager.applyTransactionEffects(analysis, {
			digest: 'test-digest',
			balanceChanges: [
				{ address: sender, coinType: HANEUL, amount: '-4' },
				{ address: extraSender, coinType: HANEUL, amount: '-3' },
				{ address: '0x3', coinType: HANEUL, amount: '7' },
			],
		} as unknown as Parameters<AutoApprovalManager['applyTransactionEffects']>[1]);

		expect(manager.getSettings()?.coinBudgets[HANEUL]).toBe('3');
		expect(manager.getState().pendingDigests).toEqual([]);
	});
