...
const HANEUL_FRAMEWORK = normalizeHaneulAddress('0x2');
...
export const balanceFlows = createAnalyzer({
	...
	analyze:
		...
		async ({ data, commands, inputs, coins, gasCoins }) => {
			...
			const sender = data.sender ? normalizeHaneulAddress(data.sender) : null;
			const gasOwner = data.gasData.owner
				? normalizeHaneulAddress(data.gasData.owner)
				: (sender ?? null);

			const normalizeAddress = (address: string | null): string | null =>
				address == null ? null : normalizeHaneulAddress(address);
			...
			const handleFrameworkMoveCall = (command: AnalyzedMoveCallCommand): boolean => {
				const pkg = normalizeHaneulAddress(command.command.package);
				if (pkg !== HANEUL_FRAMEWORK) return false;
			...
			const haneulType = normalizeStructTag('0x2::haneul::HANEUL');
			...
			const gasCoin = new TrackedBalance('coin', haneulType, gasBalance, normalizedGasOwner);
			track('gas', gasCoin);
			recordFlow(normalizedGasOwner, haneulType, -gasBalance);

			if (data.gasData.budget) {
				if (!excludeGasBudget) {
					const budget = BigInt(data.gasData.budget);
					if (paymentIsEmpty) {
						recordFlow(normalizedGasOwner, haneulType, -budget);
					} else {
