...
import { deriveObjectID, normalizeStructTag, normalizeHaneulAddress } from '@haneullabs/haneul/utils';
...
function readPureAddress(arg: AnalyzedCommandArgument | undefined): string | null {
	if (arg?.$kind !== 'Pure') return null;
	try {
		return normalizeHaneulAddress(bcs.Address.fromBase64(arg.bytes));
	} catch {
		return null;
	}
}
...
export function createPASMoveCallHandler(options: {
	...
	const packageId = normalizeHaneulAddress(options.packageId);
	const namespaceId = normalizeHaneulAddress(options.namespaceId);
	...
	return (): BalanceFlowsMoveCallHandler => {
		...
		return (command: AnalyzedMoveCallCommand, ctx: BalanceFlowsMoveCallHandlerContext): boolean => {
			if (normalizeHaneulAddress(command.command.package) !== packageId) return false;
			...
			const readAccountAddress = (arg: AnalyzedCommandArgument | undefined): string | null => {
				if (arg?.$kind === 'Object') return normalizeHaneulAddress(arg.object.objectId);
