...
import { fromBase58, fromHex, toHex } from '@haneullabs/bcs';
...
export function parseAccumulatorFieldCoinType(objectType: string): string | null {
	...
	if (
		field.address !== HANEUL_FRAMEWORK ||
	...
	if (
		valueParam.address !== HANEUL_FRAMEWORK ||
	...
	if (
		keyParam.address !== HANEUL_FRAMEWORK ||
	...
	if (
		balanceParam.address !== HANEUL_FRAMEWORK ||
