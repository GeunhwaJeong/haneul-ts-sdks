...
export const e2eLiveNetworkDryRunFlow = async (network: 'mainnet' | 'testnet') => {
	const client = new HaneulClient({ url: getFullnodeUrl(network) });
	...
	const coinDiscount = await haneulnsClient.getCoinTypeDiscount();
	...
	const expectedCoinDiscount = new Map([
		[haneulnsClient.config.coins.USDC.type.slice(2), 0],
		[haneulnsClient.config.coins.SUI.type.slice(2), 0],
		[haneulnsClient.config.coins.NS.type.slice(2), 25],
	]);
	...
	const priceInfoObjectId =
		coinConfig !== haneulnsClient.config.coins.USDC
			? (await haneulnsClient.getPriceInfoObject(tx, coinConfig.feed))[0]
			: null;
	...
	const nft = suinsTx.register({
		domain: uniqueName,
		years: 2,
		coinConfig: haneulnsClient.config.coins.SUI,
		coin: coinInput,
		priceInfoObjectId,
	});
