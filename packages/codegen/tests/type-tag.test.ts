...
function makeTypes() {
	...
	const { bcs } = require('@haneullabs/haneul/bcs');
...
describe('typeTag (runtime)', () => {
	...
	it('fills phantom holes positionally', () => {
		...
		expect(Balance.typeTag({ typeArguments: ['0x2::haneul::HANEUL'] })).toBe(
			'0x2::balance::Balance<0x2::haneul::HANEUL>',
		);
		expect(Pool.typeTag({ typeArguments: ['0x2::haneul::HANEUL', '0xb::wal::WAL'] })).toBe(
			'0xdee9::pool::Pool<0x2::haneul::HANEUL, 0xb::wal::WAL>',
		);
	});
	...
	it('uses supplied type arguments verbatim', () => {
		...
		expect(
			MapOfBalances.typeTag({
				typeArguments: ['u8', `${padded}::balance::Balance<0x2::haneul::HANEUL>`],
			}),
		).toBe(`0x2::vec_map::VecMap<u8, ${padded}::balance::Balance<0x2::haneul::HANEUL>>`);
		expect(
			MapOfBalances.typeTag({
				typeArguments: ['u8', '@haneullabs/framework::balance::Balance<0x2::haneul::HANEUL>'],
			}),
		).toBe('0x2::vec_map::VecMap<u8, @haneullabs/framework::balance::Balance<0x2::haneul::HANEUL>>');
	});
	...
	it('requires type arguments when the name has phantom holes', () => {
		...
		expect(
			MapOfBalances.typeTag({
				typeArguments: ['u8', '0x2::balance::Balance<0x2::haneul::HANEUL>'],
			}),
		).toBe('0x2::vec_map::VecMap<u8, 0x2::balance::Balance<0x2::haneul::HANEUL>>');
	...
	it('supports the package override', () => {
		...
		expect(Balance.typeTag({ package: '0x9f9', typeArguments: ['0x2::haneul::HANEUL'] })).toBe(
			'0x9f9::balance::Balance<0x2::haneul::HANEUL>',
		);
	});
	...
	it('composes through nested typeTag calls', () => {
		...
		expect(
			MapOfBalances.typeTag({
				typeArguments: ['u8', Balance.typeTag({ typeArguments: ['0x2::haneul::HANEUL'] })],
			}),
		).toBe('0x2::vec_map::VecMap<u8, 0x2::balance::Balance<0x2::haneul::HANEUL>>');
	});
	...
	it('works on MoveEnum', () => {
		...
		expect(Status.typeTag({ typeArguments: ['0x2::haneul::HANEUL'] })).toBe(
			'0xabc::status::Status<0x2::haneul::HANEUL>',
		);
	});
});
...
describe('resolveTypeTag (runtime)', () => {
	...
	it('normalizes address-only tags without resolution', async () => {
		...
		expect(await Balance.resolveTypeTag({ client, typeArguments: ['0x2::haneul::HANEUL'] })).toBe(
			`0x${'0'.repeat(63)}2::balance::Balance<0x${'0'.repeat(63)}2::haneul::HANEUL>`,
		);
	});
});
...
const t3 = Balance.typeTag({ typeArguments: ['0x2::haneul::HANEUL'] });
...
MapOfBalances.typeTag();
MapOfBalances.typeTag({ typeArguments: ['u8', '0x2::balance::Balance<0x2::haneul::HANEUL>'] });
...
Balance.typeTag({ typeArguments: [{ name: '0x2::haneul::HANEUL' }] });
...
declare const client: import('@haneullabs/haneul/client').ClientWithCoreApi;
...
void Balance.resolveTypeTag({ client, typeArguments: ['0x2::haneul::HANEUL'] });
