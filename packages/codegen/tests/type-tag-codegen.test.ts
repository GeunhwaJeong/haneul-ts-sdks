...
describe('typeTag on real codegen output', () => {
	it('builds tags for a phantom-only struct const (Container<phantom T>)', () => {
		...
		expect(registry.Container.typeTag({ typeArguments: ['0x2::haneul::HANEUL'] })).toBe(
			'@test/testpkg::registry::Container<0x2::haneul::HANEUL>',
		);
	...
	it('builds tags for a phantom-only enum const (PhantomResult<phantom T>)', () => {
		expect(registry.PhantomResult.typeTag({ typeArguments: ['0x2::haneul::HANEUL'] })).toBe(
			'@test/testpkg::registry::PhantomResult<0x2::haneul::HANEUL>',
		);
	});
	...
	it('takes the full positional list for a mixed factory (Pair<T, phantom U>)', () => {
		...
		expect(pair.typeTag({ typeArguments: ['u64', '0x2::haneul::HANEUL'] })).toBe(
			'@test/testpkg::counter::Pair<u64, 0x2::haneul::HANEUL>',
		);
	});
	...
	it('takes the full positional list for a mixed enum factory (MixedResult<phantom T, V>)', () => {
		...
		expect(mr.typeTag({ typeArguments: ['0x2::haneul::HANEUL', 'u64'] })).toBe(
			'@test/testpkg::registry::MixedResult<0x2::haneul::HANEUL, u64>',
		);
	});

	it('accepts another generated type as a type argument', () => {
		const inner = registry.Container.typeTag({ typeArguments: ['0x2::haneul::HANEUL'] });
		expect(registry.PhantomResult.typeTag({ typeArguments: [inner] })).toBe(
			'@test/testpkg::registry::PhantomResult<@test/testpkg::registry::Container<0x2::haneul::HANEUL>>',
		);
	});
});
...
describe('resolveTypeTag on real codegen output', () => {
	...
	it('resolves the package name and normalizes the result', async () => {
		...
		expect(resolved).toBe(
			`0x${'0'.repeat(61)}abc::registry::Container<0x${'0'.repeat(63)}2::haneul::HANEUL>`,
		);
	});
});
