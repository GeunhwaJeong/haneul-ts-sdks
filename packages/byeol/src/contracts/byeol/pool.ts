...
const $moduleName = '@byeol/core::pool';
...
export const BylBurned = new MoveStruct({
	name: `${$moduleName}::BylBurned<phantom BaseAsset, phantom QuoteAsset>`,
	fields: {
		pool_id: bcs.Address,
		byl_burned: bcs.u64(),
	},
 ...
 * pay_with_byl must be true, so the fee will be paid with BYL tokens.
 */
...
/**
 * Swap exact base quantity without needing a `balance_manager`. BYL quantity can
 * be overestimated. Returns three `Coin` objects: base, quote, and byl. Some base
 ...
 * BYL. Assumes balance manager has enough BYL for fees.
 */
...
/**
 * Swap exact quote quantity without needing a `balance_manager`. BYL quantity can
 * be overestimated. Returns three `Coin` objects: base, quote, and byl. Some
...
/**
 * Swap exact quantity with a `balance_manager`. Assumes fees are paid in BYL.
 * Assumes balance manager has enough BYL for fees.
 */
...
/**
 * Stake BYL tokens to the pool. The balance_manager must have enough BYL tokens.
...
/**
 * Unstake BYL tokens from the pool. The balance_manager must have enough staked
 * BYL tokens. The balance_manager's data is updated with the unstaked amount.
 ...
 * balance_manager must have enough staked BYL tokens to participate. Each
...
/**
 * Vote on a proposal. The balance_manager must have enough staked BYL tokens to
...
export interface AddBylPricePointArguments {
	targetPool: RawTransactionArgument<string>;
	referencePool: RawTransactionArgument<string>;
}
export interface AddBylPricePointOptions {
	package?: string;
	arguments:
		| AddBylPricePointArguments
...
/**
 * Adds a price point along with a timestamp to the byl price. Allows for the
 * calculation of byl price per base asset.
 */
export function addBylPricePoint(options: AddBylPricePointOptions) {
	const packageAddress = options.package ?? '@byeol/core';
	...
	return (tx: Transaction) =>
		tx.moveCall({
			package: packageAddress,
			module: 'pool',
			function: 'add_byl_price_point',
			arguments: normalizeMoveArguments(options.arguments, argumentsTypes, parameterNames),
			typeArguments: options.typeArguments,
		});
}
export interface BurnBylArguments {
	self: RawTransactionArgument<string>;
	treasuryCap: RawTransactionArgument<string>;
}
export interface BurnBylOptions {
	package?: string;
	arguments:
		| BurnBylArguments
...
/** Burns BYL tokens from the pool. Amount to burn is within history */
export function burnDeep(options: BurnBylOptions) {
	const packageAddress = options.package ?? '@byeol/core';
	const argumentsTypes = [null, null] satisfies (string | null)[];
...
/** Mint a ByeolReferral and set the additional bps for the referral. */
...
export interface UpdateByeolReferralMultiplierArguments {
	Self: RawTransactionArgument<string>;
	Referral: RawTransactionArgument<string>;
	Multiplier: RawTransactionArgument<number | bigint>;
}
export interface UpdateByeolReferralMultiplierOptions {
	package?: string;
	arguments:
		| UpdateByeolReferralMultiplierArguments
		| [
				Self: RawTransactionArgument<string>,
				Referral: RawTransactionArgument<string>,
				Multiplier: RawTransactionArgument<number | bigint>,
...
export function updateByeolReferralMultiplier(options: UpdateByeolReferralMultiplierOptions) {
	const packageAddress = options.package ?? '@byeol/core';
	const argumentsTypes = [null, null, 'u64'] satisfies (string | null)[];
	const parameterNames = ['Self', 'Referral', 'Multiplier'];
	return (tx: Transaction) =>
		tx.moveCall({
			package: packageAddress,
			module: 'pool',
			function: 'update_byeol_referral_multiplier',
			arguments: normalizeMoveArguments(options.arguments, argumentsTypes, parameterNames),
			typeArguments: options.typeArguments,
		});
}
...
/**
 * Dry run to determine the quote quantity out for a given base quantity. Uses BYL
 * token as fee.
 */
...
/**
 * Dry run to determine the base quantity out for a given quote quantity. Uses BYL
 * token as fee.
 */
 ...
 * (base_quantity_out, quote_quantity_out, byl_quantity_required) Uses BYL token
 * as fee.
 */
 ...
 * Dry run to determine the quantity out for a given base or quote quantity. Only
 * one out of base or quote quantity should be non-zero. Returns the
 * (base_quantity_out, quote_quantity_out, byl_quantity_required) Uses input token
 * as fee.
 */
 ...
 * byl_quantity_required) Returns (0, 0, 0) if insufficient liquidity or if result
 ...
 * Returns (actual_base_quantity_out, quote_quantity_in, byl_quantity_required)
...
export interface GetOrderBylPriceArguments {
	self: RawTransactionArgument<string>;
}
export interface GetOrderBylPriceOptions {
	package?: string;
	arguments: GetOrderBylPriceArguments | [self: RawTransactionArgument<string>];
	typeArguments: [string, string];
}
/** Return the BYL price for the pool. */
export function getOrderBylPrice(options: GetOrderBylPriceOptions) {
	const packageAddress = options.package ?? '@byeol/core';
	const argumentsTypes = [null] satisfies (string | null)[];
	const parameterNames = ['self'];
	return (tx: Transaction) =>
		tx.moveCall({
			package: packageAddress,
			module: 'pool',
			function: 'get_order_byl_price',
			arguments: normalizeMoveArguments(options.arguments, argumentsTypes, parameterNames),
			typeArguments: options.typeArguments,
		});
}
export interface GetOrderBylRequiredArguments {
	self: RawTransactionArgument<string>;
	baseQuantity: RawTransactionArgument<number | bigint>;
	price: RawTransactionArgument<number | bigint>;
}
export interface GetOrderBylRequiredOptions {
	package?: string;
	arguments:
		| GetOrderBylRequiredArguments
		| [
				self: RawTransactionArgument<string>,
				baseQuantity: RawTransactionArgument<number | bigint>,
				price: RawTransactionArgument<number | bigint>,
...
/**
 * Returns the byl required for an order if it's taker or maker given quantity and
 * price Does not account for discounted taker fees Returns (byl_required_taker,
 * byl_required_maker)
 */
export function getOrderBylRequired(options: GetOrderBylRequiredOptions) {
	const packageAddress = options.package ?? '@byeol/core';
	...
	return (tx: Transaction) =>
		tx.moveCall({
			package: packageAddress,
			module: 'pool',
			function: 'get_order_byl_required',
			arguments: normalizeMoveArguments(options.arguments, argumentsTypes, parameterNames),
			typeArguments: options.typeArguments,
		});
}
 ...
 * (base_quantity, quote_quantity, byl_quantity)
 */
