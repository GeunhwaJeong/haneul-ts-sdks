...
import { ByeolContract } from './transactions/byeol.js';
import { ByeolAdminContract } from './transactions/byeolAdmin.js';
...
import { ByeolConfig } from './utils/config.js';
import type { CoinMap, ByeolPackageIds, PoolMap } from './utils/constants.js';
import { normalizeHaneulAddress } from '@haneullabs/haneul/utils';

export interface ByeolCompatibleClient extends ClientWithCoreApi {}

export interface ByeolOptions<Name = 'byeol'> {
	address: string;
...
export interface ByeolClientOptions extends ByeolOptions {
	client: ByeolCompatibleClient;
	network: HaneulClientTypes.Network;
}

export function byeol<Name extends string = 'byeol'>({
	name = 'byeol' as Name,
	...options
}: ByeolOptions<Name>): HaneulClientRegistration<ByeolCompatibleClient, Name, ByeolClient> {
	return {
		name,
		register: (client) => {
			return new ByeolClient({
				client,
...
/**
 * ByeolClient class for managing Byeol operations.
 */
export class ByeolClient {
	...
	/**
	 * Creates a new ByeolClient instance
	 */
	constructor({
		client,
		address,
		...
		this.byeol = new ByeolContract(config);
		this.byeolAdmin = new ByeolAdminContract(config);
	...
	getOrderBylRequired(
		...
		return this.#quantityQueries.getOrderBylRequired(poolKey, baseQuantity, price);
	}
	...
	getPoolBylPrice(poolKey: string): Promise<PoolBylPrice> {
		return this.#accountQueries.getPoolBylPrice(poolKey);
	}
	...
	isByeolPoolAllowed(coinKey: string, byeolPoolId: string): Promise<boolean> {
		return this.#marginPoolQueries.isByeolPoolAllowed(coinKey, byeolPoolId);
	}
	...
	getMarginManagerByeolPool(marginManagerKey: string): Promise<string> {
		return this.#marginManagerQueries.getMarginManagerByeolPool(marginManagerKey);
	}
	...
	getMarginManagerBylBalance(marginManagerKey: string, decimals: number = 6): Promise<string> {
		return this.#marginManagerQueries.getMarginManagerBylBalance(marginManagerKey, decimals);
	}
