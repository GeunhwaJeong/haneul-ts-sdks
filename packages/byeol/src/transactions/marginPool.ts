...
export class MarginPoolContract {
	...
	/**
	 * @param {ByeolConfig} config Configuration for MarginPoolContract
	 */
	...
	/**
	 * @description Check if a byeol pool is allowed for borrowing
	 * @param {string} coinKey The key to identify the margin pool
	 * @param {string} byeolPoolId The ID of the byeol pool
	 * @returns A function that takes a Transaction object
	 */
	byeolPoolAllowed = (coinKey: string, byeolPoolId: string) => (tx: Transaction) => {
		const marginPool = this.#config.getMarginPool(coinKey);
		return tx.moveCall({
			target: `${this.#config.MARGIN_PACKAGE_ID}::margin_pool::byeol_pool_allowed`,
			arguments: [tx.object(marginPool.address), tx.pure.id(byeolPoolId)],
			typeArguments: [marginPool.type],
		});
	};
