...
import {
	...
	GAS_BUDGET,
	MAX_TIMESTAMP,
	POOL_CREATION_FEE_BYL,
} from '../utils/config.js';
import { convertQuantity, convertPrice, convertRate } from '../utils/conversion.js';

/**
 * ByeolContract class for managing Byeol operations.
 */
export class ByeolContract {
	#config: ByeolConfig;

	/**
	 * @param {ByeolConfig} config Configuration for ByeolContract
	 */
	...
	placeLimitOrder = (params: PlaceLimitOrderParams) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.BYEOL_PACKAGE_ID}::pool::place_limit_order`,
			arguments: [
	...
	placeMarketOrder = (params: PlaceMarketOrderParams) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.BYEOL_PACKAGE_ID}::pool::place_market_order`,
			arguments: [
				tx.object(pool.address),
				tx.object(balanceManager.address),
				tradeProof,
	...
	modifyOrder =
		...
		(tx: Transaction) => {
			...
			tx.moveCall({
				target: `${this.#config.BYEOL_PACKAGE_ID}::pool::modify_order`,
				arguments: [
					tx.object(pool.address),
					tx.object(balanceManager.address),
					tradeProof,
	...
	cancelOrder =
		(poolKey: string, balanceManagerKey: string, orderId: string) => (tx: Transaction) => {
			...
			tx.moveCall({
				target: `${this.#config.BYEOL_PACKAGE_ID}::pool::cancel_order`,
				arguments: [
					tx.object(pool.address),
					tx.object(balanceManager.address),
					tradeProof,
	...
	cancelOrders =
		(poolKey: string, balanceManagerKey: string, orderIds: string[]) => (tx: Transaction) => {
			...
			tx.moveCall({
				target: `${this.#config.BYEOL_PACKAGE_ID}::pool::cancel_orders`,
				arguments: [
					tx.object(pool.address),
					tx.object(balanceManager.address),
					tradeProof,
	...
	cancelLiveOrder =
		(poolKey: string, balanceManagerKey: string, orderId: string) => (tx: Transaction) => {
			...
			tx.moveCall({
				target: `${this.#config.BYEOL_PACKAGE_ID}::pool::cancel_live_order`,
				arguments: [
					tx.object(pool.address),
					tx.object(balanceManager.address),
					tradeProof,
	...
	cancelLiveOrders =
		(poolKey: string, balanceManagerKey: string, orderIds: string[]) => (tx: Transaction) => {
			...
			tx.moveCall({
				target: `${this.#config.BYEOL_PACKAGE_ID}::pool::cancel_live_orders`,
				arguments: [
					tx.object(pool.address),
					tx.object(balanceManager.address),
					tradeProof,
	...
	cancelAllOrders = (poolKey: string, balanceManagerKey: string) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.BYEOL_PACKAGE_ID}::pool::cancel_all_orders`,
			arguments: [
				tx.object(pool.address),
				tx.object(balanceManager.address),
				tradeProof,
	...
	withdrawSettledAmounts = (poolKey: string, balanceManagerKey: string) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.BYEOL_PACKAGE_ID}::pool::withdraw_settled_amounts`,
	...
	withdrawSettledAmountsPermissionless =
		(poolKey: string, balanceManagerKey: string) => (tx: Transaction) => {
			...
			tx.moveCall({
				target: `${this.#config.BYEOL_PACKAGE_ID}::pool::withdraw_settled_amounts_permissionless`,
	...
	/**
	 * @description Add a byl price point for a target pool using a reference pool
	 ...
	 * @returns A function that takes a Transaction object
	 */
	addBylPricePoint = (targetPoolKey: string, referencePoolKey: string) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.BYEOL_PACKAGE_ID}::pool::add_byl_price_point`,
			arguments: [
	...
	claimRebates = (poolKey: string, balanceManagerKey: string) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.BYEOL_PACKAGE_ID}::pool::claim_rebates`,
			arguments: [tx.object(pool.address), tx.object(balanceManager.address), tradeProof],
			typeArguments: [baseCoin.type, quoteCoin.type],
		});
	};
	...
	mintReferral = (poolKey: string, multiplier: number) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.BYEOL_PACKAGE_ID}::pool::mint_referral`,
	...
	/**
	 * @description Update the referral multiplier for a pool (ByeolPoolReferral)
	 * @param {string} poolKey The key to identify the pool
	 * @param {string} referral The referral (ByeolPoolReferral) to update
	 * @param {number} multiplier The multiplier for the referral
	 * @returns A function that takes a Transaction object
	 */
	updatePoolReferralMultiplier =
		(poolKey: string, referral: string, multiplier: number) => (tx: Transaction) => {
			...
			tx.moveCall({
				target: `${this.#config.BYEOL_PACKAGE_ID}::pool::update_pool_referral_multiplier`,
	...
	/**
	 * @description Claim the rewards for a referral (ByeolPoolReferral)
	 * @param {string} poolKey The key to identify the pool
	 * @param {string} referral The referral (ByeolPoolReferral) to claim the rewards for
	 * @returns A function that takes a Transaction object
	 */
	claimPoolReferralRewards = (poolKey: string, referral: string) => (tx: Transaction) => {
		...
		const [baseRewards, quoteRewards, deepRewards] = tx.moveCall({
			target: `${this.#config.BYEOL_PACKAGE_ID}::pool::claim_pool_referral_rewards`,
	...
	updatePoolAllowedVersions = (poolKey: string) => (tx: Transaction) => {
		...
		const quoteCoin = this.#config.getCoin(pool.quoteCoin);
		tx.moveCall({
			target: `${this.#config.BYEOL_PACKAGE_ID}::pool::update_pool_allowed_versions`,
	...
	getOrder = (poolKey: string, orderId: string) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.BYEOL_PACKAGE_ID}::pool::get_order`,
	...
	getOrders = (poolKey: string, orderIds: string[]) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.BYEOL_PACKAGE_ID}::pool::get_orders`,
	...
	/**
	 * @description Burn BYL tokens from the pool
	 * @param {string} poolKey The key to identify the pool
	 * @returns A function that takes a Transaction object
	 */
	burnDeep = (poolKey: string) => (tx: Transaction) => {
		...
		const quoteCoin = this.#config.getCoin(pool.quoteCoin);
		tx.moveCall({
			target: `${this.#config.BYEOL_PACKAGE_ID}::pool::burn_byl`,
			arguments: [tx.object(pool.address), tx.object(this.#config.BYL_TREASURY_ID)],
			typeArguments: [baseCoin.type, quoteCoin.type],
		});
	};
	...
	midPrice = (poolKey: string) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.BYEOL_PACKAGE_ID}::pool::mid_price`,
	...
	whitelisted = (poolKey: string) => (tx: Transaction) => {
		...
		const quoteCoin = this.#config.getCoin(pool.quoteCoin);
		tx.moveCall({
			target: `${this.#config.BYEOL_PACKAGE_ID}::pool::whitelisted`,
	...
	getQuoteQuantityOut = (poolKey: string, baseQuantity: number | bigint) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.BYEOL_PACKAGE_ID}::pool::get_quote_quantity_out`,
			arguments: [
				tx.object(pool.address),
	...
	getBaseQuantityOut = (poolKey: string, quoteQuantity: number | bigint) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.BYEOL_PACKAGE_ID}::pool::get_base_quantity_out`,
			arguments: [
				tx.object(pool.address),
	...
	getQuantityOut =
		...
		(tx: Transaction) => {
			...
			tx.moveCall({
				target: `${this.#config.BYEOL_PACKAGE_ID}::pool::get_quantity_out`,
				arguments: [
					tx.object(pool.address),
					tx.pure.u64(convertQuantity(baseQuantity, baseCoin.scalar)),
					tx.pure.u64(convertQuantity(quoteQuantity, quoteScalar)),
	...
	accountOpenOrders = (poolKey: string, managerKey: string) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.BYEOL_PACKAGE_ID}::pool::account_open_orders`,
	...
	getLevel2Range =
		...
		(tx: Transaction) => {
			...
			tx.moveCall({
				target: `${this.#config.BYEOL_PACKAGE_ID}::pool::get_level2_range`,
				arguments: [
					tx.object(pool.address),
	...
	getLevel2TicksFromMid = (poolKey: string, tickFromMid: number) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.BYEOL_PACKAGE_ID}::pool::get_level2_ticks_from_mid`,
	...
	vaultBalances = (poolKey: string) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.BYEOL_PACKAGE_ID}::pool::vault_balances`,
			arguments: [tx.object(pool.address)],
			typeArguments: [baseCoin.type, quoteCoin.type],
		});
	};
	...
	getPoolIdByAssets = (baseType: string, quoteType: string) => (tx: Transaction) => {
		tx.moveCall({
			target: `${this.#config.BYEOL_PACKAGE_ID}::pool::get_pool_id_by_asset`,
	...
	swapExactBaseForQuote = (params: SwapParams) => (tx: Transaction) => {
		...
		const pool = this.#config.getPool(poolKey);
		const deepCoinType = this.#config.getCoin('BYL').type;
		const baseCoin = this.#config.getCoin(pool.baseCoin);
		const quoteCoin = this.#config.getCoin(pool.quoteCoin);
		...
		const deepCoin =
			params.deepCoin ??
			coinWithBalance({ type: deepCoinType, balance: convertQuantity(deepAmount, BYL_SCALAR) });
		...
		const [baseCoinResult, quoteCoinResult, deepCoinResult] = tx.moveCall({
			target: `${this.#config.BYEOL_PACKAGE_ID}::pool::swap_exact_base_for_quote`,
			arguments: [
				tx.object(pool.address),
				baseCoinInput,
				deepCoin,
	...
	swapExactQuoteForBase = (params: SwapParams) => (tx: Transaction) => {
		...
		const [baseCoinResult, quoteCoinResult, deepCoinResult] = tx.moveCall({
			target: `${this.#config.BYEOL_PACKAGE_ID}::pool::swap_exact_quote_for_base`,
			arguments: [
				tx.object(pool.address),
				quoteCoinInput,
				deepCoin,
	...
	swapExactQuantity = (params: SwapParams & { isBaseToCoin: boolean }) => (tx: Transaction) => {
		...
		const [baseCoinResult, quoteCoinResult, deepCoinResult] = tx.moveCall({
			target: `${this.#config.BYEOL_PACKAGE_ID}::pool::swap_exact_quantity`,
			arguments: [
				tx.object(pool.address),
				baseCoinInput,
				quoteCoinInput,
	...
	swapExactBaseForQuoteWithManager = (params: SwapWithManagerParams) => (tx: Transaction) => {
		...
		const [baseCoinResult, quoteCoinResult] = tx.moveCall({
			target: `${this.#config.BYEOL_PACKAGE_ID}::pool::swap_exact_base_for_quote_with_manager`,
			arguments: [
				tx.object(pool.address),
				tx.object(balanceManager.address),
				tx.object(tradeCap),
	...
	swapExactQuoteForBaseWithManager = (params: SwapWithManagerParams) => (tx: Transaction) => {
		...
		const [baseCoinResult, quoteCoinResult] = tx.moveCall({
			target: `${this.#config.BYEOL_PACKAGE_ID}::pool::swap_exact_quote_for_base_with_manager`,
			arguments: [
				tx.object(pool.address),
				tx.object(balanceManager.address),
				tx.object(tradeCap),
	...
	swapExactQuantityWithManager =
		(params: SwapWithManagerParams & { isBaseToCoin: boolean }) => (tx: Transaction) => {
			...
			const [baseCoinResult, quoteCoinResult] = tx.moveCall({
				target: `${this.#config.BYEOL_PACKAGE_ID}::pool::swap_exact_quantity_with_manager`,
				arguments: [
					tx.object(pool.address),
					tx.object(balanceManager.address),
					tx.object(tradeCap),
	...
	createPermissionlessPool = (params: CreatePermissionlessPoolParams) => (tx: Transaction) => {
		...
		const deepCoinInput =
			...
			coinWithBalance({
				type: deepCoinType,
				balance: POOL_CREATION_FEE_BYL,
			});

		tx.moveCall({
			target: `${this.#config.BYEOL_PACKAGE_ID}::pool::create_permissionless_pool`,
			arguments: [
	...
	poolTradeParams = (poolKey: string) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.BYEOL_PACKAGE_ID}::pool::pool_trade_params`,
			arguments: [tx.object(pool.address)],
			typeArguments: [baseCoin.type, quoteCoin.type],
		});
	};
	...
	poolBookParams = (poolKey: string) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.BYEOL_PACKAGE_ID}::pool::pool_book_params`,
			arguments: [tx.object(pool.address)],
			typeArguments: [baseCoin.type, quoteCoin.type],
		});
	};
	...
	account = (poolKey: string, managerKey: string) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.BYEOL_PACKAGE_ID}::pool::account`,
	...
	lockedBalance = (poolKey: string, managerKey: string) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.BYEOL_PACKAGE_ID}::pool::locked_balance`,
			arguments: [tx.object(pool.address), tx.object(managerId)],
			typeArguments: [baseCoin.type, quoteCoin.type],
		});
	};
	...
	/**
	 * @description Get the BYL price conversion for a pool
	 * @param {string} poolKey The key to identify the pool
	 * @returns A function that takes a Transaction object
	 */
	getPoolBylPrice = (poolKey: string) => (tx: Transaction) => {
		const pool = this.#config.getPool(poolKey);
		const baseCoin = this.#config.getCoin(pool.baseCoin);
		const quoteCoin = this.#config.getCoin(pool.quoteCoin);

		tx.moveCall({
			target: `${this.#config.BYEOL_PACKAGE_ID}::pool::get_order_byl_price`,
			arguments: [tx.object(pool.address)],
			typeArguments: [baseCoin.type, quoteCoin.type],
		});
	};
	...
	getBalanceManagerIds = (owner: string) => (tx: Transaction) => {
		tx.moveCall({
			target: `${this.#config.BYEOL_PACKAGE_ID}::registry::get_balance_manager_ids`,
			arguments: [tx.object(this.#config.REGISTRY_ID), tx.pure.address(owner)],
		});
	};

	/**
	 * @description Get the balances for a referral (ByeolPoolReferral)
	 * @param {string} poolKey The key to identify the pool
	 * @param {string} referral The referral (ByeolPoolReferral) to get the balances for
	 * @returns A function that takes a Transaction object
	 */
	getPoolReferralBalances = (poolKey: string, referral: string) => (tx: Transaction) => {
		...
		return tx.moveCall({
			target: `${this.#config.BYEOL_PACKAGE_ID}::pool::get_pool_referral_balances`,
			arguments: [tx.object(pool.address), tx.object(referral)],
			typeArguments: [baseCoin.type, quoteCoin.type],
		});
	};
	...
	/**
	 * @description Get the multiplier for a referral (ByeolPoolReferral)
	 * @param {string} poolKey The key to identify the pool
	 * @param {string} referral The referral (ByeolPoolReferral) to get the multiplier for
	 * @returns A function that takes a Transaction object
	 */
	poolReferralMultiplier = (poolKey: string, referral: string) => (tx: Transaction) => {
		...
		return tx.moveCall({
			target: `${this.#config.BYEOL_PACKAGE_ID}::pool::pool_referral_multiplier`,
			arguments: [tx.object(pool.address), tx.object(referral)],
			typeArguments: [baseCoin.type, quoteCoin.type],
		});
	};
	...
	stablePool = (poolKey: string) => (tx: Transaction) => {
		...
		return tx.moveCall({
			target: `${this.#config.BYEOL_PACKAGE_ID}::pool::stable_pool`,
			arguments: [tx.object(pool.address)],
			typeArguments: [baseCoin.type, quoteCoin.type],
		});
	};
	...
	registeredPool = (poolKey: string) => (tx: Transaction) => {
		...
		return tx.moveCall({
			target: `${this.#config.BYEOL_PACKAGE_ID}::pool::registered_pool`,
			arguments: [tx.object(pool.address)],
			typeArguments: [baseCoin.type, quoteCoin.type],
		});
	};
	...
	getQuoteQuantityOutInputFee =
		(poolKey: string, baseQuantity: number | bigint) => (tx: Transaction) => {
			...
			return tx.moveCall({
				target: `${this.#config.BYEOL_PACKAGE_ID}::pool::get_quote_quantity_out_input_fee`,
				arguments: [
					tx.object(pool.address),
					tx.pure.u64(convertQuantity(baseQuantity, baseCoin.scalar)),
					tx.object.clock(),
	...
	getBaseQuantityOutInputFee =
		(poolKey: string, quoteQuantity: number | bigint) => (tx: Transaction) => {
			...
			return tx.moveCall({
				target: `${this.#config.BYEOL_PACKAGE_ID}::pool::get_base_quantity_out_input_fee`,
				arguments: [
					tx.object(pool.address),
	...
	getQuantityOutInputFee =
		...
		(tx: Transaction) => {
			...
			return tx.moveCall({
				target: `${this.#config.BYEOL_PACKAGE_ID}::pool::get_quantity_out_input_fee`,
				arguments: [
					tx.object(pool.address),
					tx.pure.u64(convertQuantity(baseQuantity, baseCoin.scalar)),
					tx.pure.u64(convertQuantity(quoteQuantity, quoteCoin.scalar)),
	 ...
	 * @param {boolean} payWithDeep Whether to pay fees with BYL
	 * @returns A function that takes a Transaction object
	 */
	getBaseQuantityIn =
		...
		(tx: Transaction) => {
			...
			return tx.moveCall({
				target: `${this.#config.BYEOL_PACKAGE_ID}::pool::get_base_quantity_in`,
				arguments: [
					tx.object(pool.address),
	...
	getQuoteQuantityIn =
		...
		(tx: Transaction) => {
			...
			return tx.moveCall({
				target: `${this.#config.BYEOL_PACKAGE_ID}::pool::get_quote_quantity_in`,
				arguments: [
					tx.object(pool.address),
	...
	getAccountOrderDetails = (poolKey: string, managerKey: string) => (tx: Transaction) => {
		...
		return tx.moveCall({
			target: `${this.#config.BYEOL_PACKAGE_ID}::pool::get_account_order_details`,
			arguments: [tx.object(pool.address), tx.object(manager.address)],
			typeArguments: [baseCoin.type, quoteCoin.type],
		});
	};
	...
	/**
	 * @description Get the BYL required for an order
	 * @param {string} poolKey The key to identify the pool
	...
	getOrderBylRequired =
		...
		(tx: Transaction) => {
			...
			return tx.moveCall({
				target: `${this.#config.BYEOL_PACKAGE_ID}::pool::get_order_byl_required`,
	...
	accountExists = (poolKey: string, managerKey: string) => (tx: Transaction) => {
		...
		return tx.moveCall({
			target: `${this.#config.BYEOL_PACKAGE_ID}::pool::account_exists`,
			arguments: [tx.object(pool.address), tx.object(manager.address)],
			typeArguments: [baseCoin.type, quoteCoin.type],
		});
	};
	...
	poolTradeParamsNext = (poolKey: string) => (tx: Transaction) => {
		...
		return tx.moveCall({
			target: `${this.#config.BYEOL_PACKAGE_ID}::pool::pool_trade_params_next`,
			arguments: [tx.object(pool.address)],
			typeArguments: [baseCoin.type, quoteCoin.type],
		});
	};
	...
	quorum = (poolKey: string) => (tx: Transaction) => {
		...
		return tx.moveCall({
			target: `${this.#config.BYEOL_PACKAGE_ID}::pool::quorum`,
			arguments: [tx.object(pool.address)],
			typeArguments: [baseCoin.type, quoteCoin.type],
		});
	};
	...
	poolId = (poolKey: string) => (tx: Transaction) => {
		...
		return tx.moveCall({
			target: `${this.#config.BYEOL_PACKAGE_ID}::pool::id`,
			arguments: [tx.object(pool.address)],
			typeArguments: [baseCoin.type, quoteCoin.type],
		});
	};
	...
	canPlaceLimitOrder = (params: CanPlaceLimitOrderParams) => (tx: Transaction) => {
		...
		return tx.moveCall({
			target: `${this.#config.BYEOL_PACKAGE_ID}::pool::can_place_limit_order`,
			arguments: [
				tx.object(pool.address),
	...
	canPlaceMarketOrder = (params: CanPlaceMarketOrderParams) => (tx: Transaction) => {
		...
		return tx.moveCall({
			target: `${this.#config.BYEOL_PACKAGE_ID}::pool::can_place_market_order`,
			arguments: [
				tx.object(pool.address),
				tx.object(manager.address),
				tx.pure.u64(inputQuantity),
	...
	checkMarketOrderParams = (poolKey: string, quantity: number | bigint) => (tx: Transaction) => {
		...
		return tx.moveCall({
			target: `${this.#config.BYEOL_PACKAGE_ID}::pool::check_market_order_params`,
	...
	checkLimitOrderParams =
		...
		(tx: Transaction) => {
			...
			return tx.moveCall({
				target: `${this.#config.BYEOL_PACKAGE_ID}::pool::check_limit_order_params`,
				arguments: [
					tx.object(pool.address),
					tx.pure.u64(inputPrice),
					tx.pure.u64(inputQuantity),
