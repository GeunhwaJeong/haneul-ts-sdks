...
/**
 * GovernanceContract class for managing governance operations in Byeol.
 */
export class GovernanceContract {
	#config: ByeolConfig;

	/**
	 * @param {ByeolConfig} config Configuration for GovernanceContract
	 */
	...
	stake =
		(poolKey: string, balanceManagerKey: string, stakeAmount: number) => (tx: Transaction) => {
			...
			const stakeInput = convertQuantity(stakeAmount, BYL_SCALAR);

			tx.moveCall({
				target: `${this.#config.BYEOL_PACKAGE_ID}::pool::stake`,
				arguments: [
	...
	unstake = (poolKey: string, balanceManagerKey: string) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.BYEOL_PACKAGE_ID}::pool::unstake`,
	...
	submitProposal = (params: ProposalParams) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.BYEOL_PACKAGE_ID}::pool::submit_proposal`,
			arguments: [
				tx.object(pool.address),
				tx.object(balanceManager.address),
				tradeProof,
				...
				tx.pure.u64(convertQuantity(stakeRequired, BYL_SCALAR)),
			],
			typeArguments: [baseCoin.type, quoteCoin.type],
		});
	};
	...
	vote = (poolKey: string, balanceManagerKey: string, proposal_id: string) => (tx: Transaction) => {
		...
		tx.moveCall({
			target: `${this.#config.BYEOL_PACKAGE_ID}::pool::vote`,
			arguments: [
				tx.object(pool.address),
				tx.object(balanceManager.address),
				tradeProof,
