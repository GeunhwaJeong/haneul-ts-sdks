...
export class MarginAdminContract {
	...
	/**
	 * @param {ByeolConfig} config Configuration for MarginAdminContract
	 */
	...
	/**
	 * @description Register a byeol pool
	...
	registerByeolPool =
		(poolKey: string, poolConfig: TransactionArgument) => (tx: Transaction) => {
			...
			tx.moveCall({
				target: `${this.#config.MARGIN_PACKAGE_ID}::margin_registry::register_byeol_pool`,
				arguments: [
					tx.object(this.#config.MARGIN_REGISTRY_ID),
					tx.object(this.#marginAdminCap()),
	...
	/**
	 * @description Enable a byeol pool for margin trading
	 * @param {string} poolKey The key of the pool to be enabled
	 * @returns A function that takes a Transaction object
	 */
	enableByeolPool = (poolKey: string) => (tx: Transaction) => {
		const pool = this.#config.getPool(poolKey);
		const baseCoin = this.#config.getCoin(pool.baseCoin);
		const quoteCoin = this.#config.getCoin(pool.quoteCoin);
		tx.moveCall({
			target: `${this.#config.MARGIN_PACKAGE_ID}::margin_registry::enable_byeol_pool`,
			arguments: [
				tx.object(this.#config.MARGIN_REGISTRY_ID),
				tx.object(this.#marginAdminCap()),
				tx.object(pool.address),
	...
	/**
	 * @description Disable a byeol pool from margin trading
	 * @param {string} poolKey The key of the pool to be disabled
	 * @returns A function that takes a Transaction object
	 */
	disableByeolPool = (poolKey: string) => (tx: Transaction) => {
		const pool = this.#config.getPool(poolKey);
		const baseCoin = this.#config.getCoin(pool.baseCoin);
		const quoteCoin = this.#config.getCoin(pool.quoteCoin);
		tx.moveCall({
			target: `${this.#config.MARGIN_PACKAGE_ID}::margin_registry::disable_byeol_pool`,
			arguments: [
				tx.object(this.#config.MARGIN_REGISTRY_ID),
				tx.object(this.#marginAdminCap()),
				tx.object(pool.address),
