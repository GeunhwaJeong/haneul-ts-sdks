...
/**
 * ByeolMaintainerContract class for managing maintainer actions.
 */
export class MarginMaintainerContract {
	#config: ByeolConfig;

	/**
	 * @param {ByeolConfig} config Configuration for MarginMaintainerContract
	 */
	...
	/**
	 * @description Enable a byeol pool for loan
	 * @param {string} byeolPoolKey The key to identify the byeol pool
	 * @param {string} coinKey The key to identify the margin pool
	 * @param {TransactionObjectArgument} marginPoolCap The margin pool cap
	 * @returns A function that takes a Transaction object
	 */
	enableByeolPoolForLoan =
		(byeolPoolKey: string, coinKey: string, marginPoolCap: TransactionObjectArgument) =>
		(tx: Transaction) => {
			const byeolPool = this.#config.getPool(byeolPoolKey);
			const marginPool = this.#config.getMarginPool(coinKey);
			tx.moveCall({
				target: `${this.#config.MARGIN_PACKAGE_ID}::margin_pool::enable_byeol_pool_for_loan`,
				arguments: [
					tx.object(marginPool.address),
					tx.object(this.#config.MARGIN_REGISTRY_ID),
					tx.pure.id(byeolPool.address),
	...
	/**
	 * @description Disable a byeol pool for loan
	 * @param {string} byeolPoolKey The key to identify the byeol pool
	 * @param {string} coinKey The key to identify the margin pool
	 * @param {TransactionObjectArgument} marginPoolCap The margin pool cap
	 * @returns A function that takes a Transaction object
	 */
	disableByeolPoolForLoan =
		(byeolPoolKey: string, coinKey: string, marginPoolCap: TransactionObjectArgument) =>
		(tx: Transaction) => {
			const byeolPool = this.#config.getPool(byeolPoolKey);
			const marginPool = this.#config.getMarginPool(coinKey);
			tx.moveCall({
				target: `${this.#config.MARGIN_PACKAGE_ID}::margin_pool::disable_byeol_pool_for_loan`,
				arguments: [
					tx.object(marginPool.address),
					tx.object(this.#config.MARGIN_REGISTRY_ID),
					tx.pure.id(byeolPool.address),
