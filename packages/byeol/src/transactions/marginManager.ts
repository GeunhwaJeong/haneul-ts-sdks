...
export class MarginManagerContract {
	...
	/**
	 * @param {ByeolConfig} config Configuration for MarginManagerContract
	 */
	 ...
	 * if the manager holds any outstanding debt or base/quote/BYL balance.
	 * @param {string} managerKey The key to identify the margin manager
	 * @returns A function that takes a Transaction object
	 */
	...
	depositDuringInitialization = (params: DepositDuringInitParams) => (tx: Transaction) => {
		...
		// Get the deposit coin from config using the coinType key (e.g., 'HANEUL', 'DBUSDC', 'BYL')
	...
	/**
	 * @description Deposit byl into a margin manager
	 * @param {DepositParams} params The deposit parameters
	 * @returns A function that takes a Transaction object
	 */
	...
	/**
	 * @description Withdraw byl from a margin manager
	 * @param {string} managerKey The key to identify the manager
	 * @param {number} amount The amount to withdraw
	 * @returns A function that takes a Transaction object
	 */
	...
	/**
	 * @description Set the referral for a margin manager (ByeolPoolReferral)
	 * @param {string} managerKey The key to identify the margin manager
	 * @param {string} referral The referral (ByeolPoolReferral) to set
	 * @returns A function that takes a Transaction object
	 */
	...
	/**
	 * @description Get the Byeol pool ID associated with a margin manager
	 * @param {string} poolKey The key to identify the pool
	 * @param {string} marginManagerId The ID of the margin manager
	 * @returns A function that takes a Transaction object
	 */
	byeolPool = (poolKey: string, marginManagerId: string) => (tx: Transaction) => {
		const pool = this.#config.getPool(poolKey);
		const baseCoin = this.#config.getCoin(pool.baseCoin);
		const quoteCoin = this.#config.getCoin(pool.quoteCoin);
		return tx.moveCall({
			target: `${this.#config.MARGIN_PACKAGE_ID}::margin_manager::byeol_pool`,
			arguments: [tx.object(marginManagerId)],
			typeArguments: [baseCoin.type, quoteCoin.type],
		});
	};
	 ...
	 * @param {string} marginManagerId The ID of the margin manager
	 * @returns A function that takes a Transaction object
	 * @returns Returns (manager_id, byeol_pool_id, risk_ratio, base_asset, quote_asset,
	...
	/**
	 * @description Get the BYL token balance of a margin manager
	 * @param {string} poolKey The key to identify the pool
	 * @param {string} marginManagerId The ID of the margin manager
	 * @returns A function that takes a Transaction object
	 */
	deepBalance = (poolKey: string, marginManagerId: string) => (tx: Transaction) => {
		...
		const quoteCoin = this.#config.getCoin(pool.quoteCoin);
		return tx.moveCall({
			target: `${this.#config.MARGIN_PACKAGE_ID}::margin_manager::byl_balance`,
			arguments: [tx.object(marginManagerId)],
			typeArguments: [baseCoin.type, quoteCoin.type],
		});
	};
	...
	/**
	 * @description Get locked balances (base, quote, byl) for the margin
	 ...
	 * @param {boolean} payWithDeep Whether to pay fees in BYL
