...
export class OrderQueries {
	...
	async accountOpenOrders(poolKey: string, managerKey: string): Promise<string[]> {
		...
		tx.add(this.#ctx.byeol.accountOpenOrders(poolKey, managerKey));
	...
	async getOrder(poolKey: string, orderId: string): Promise<ReturnType<typeof Order.parse> | null> {
		...
		tx.add(this.#ctx.byeol.getOrder(poolKey, orderId));
		const res = await this.#ctx.client.core.simulateTransaction({
			transaction: tx,
			include: { commandResults: true, effects: true },
		});
	...
	async getOrderNormalized(poolKey: string, orderId: string) {
		...
		try {
			...
			const normalizedOrderInfo = {
				...
				order_byl_price: {
					...orderInfo.order_byl_price,
					byl_per_asset: String(
						(Number(orderInfo.order_byl_price.byl_per_asset) / BYL_SCALAR).toFixed(9),
					),
				},
				isBid,
	...
	async getOrders(
		...
		tx.add(this.#ctx.byeol.getOrders(poolKey, orderIds));
		const res = await this.#ctx.client.core.simulateTransaction({
			transaction: tx,
			include: { commandResults: true, effects: true },
		});
	...
	async getLevel2Range(
		...
		tx.add(this.#ctx.byeol.getLevel2Range(poolKey, priceLow, priceHigh, isBid));
		const res = await this.#ctx.client.core.simulateTransaction({
			transaction: tx,
			include: { commandResults: true, effects: true },
		});
	...
	async getLevel2TicksFromMid(poolKey: string, ticks: number): Promise<Level2TicksFromMid> {
		...
		tx.add(this.#ctx.byeol.getLevel2TicksFromMid(poolKey, ticks));
		const res = await this.#ctx.client.core.simulateTransaction({
			transaction: tx,
			include: { commandResults: true, effects: true },
		});
	...
	async getAccountOrderDetails(
		...
		const tx = new Transaction();
		tx.setSender(this.#ctx.address);
		tx.add(this.#ctx.byeol.getAccountOrderDetails(poolKey, managerKey));
