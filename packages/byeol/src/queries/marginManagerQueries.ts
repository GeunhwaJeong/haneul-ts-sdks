...
export class MarginManagerQueries {
	...
	async getMarginManagerByeolPool(marginManagerKey: string): Promise<string> {
		const manager = this.#ctx.config.getMarginManager(marginManagerKey);
		const tx = new Transaction();
		tx.setSender(this.#ctx.address);
		tx.add(this.#ctx.marginManager.byeolPool(manager.poolKey, manager.address));
	...
	async getMarginManagerState(
		...
		const byeolPoolId = normalizeHaneulAddress(
	...
	async getMarginManagerBylBalance(
		marginManagerKey: string,
		decimals: number = 6,
	): Promise<string> {
		const manager = this.#ctx.config.getMarginManager(marginManagerKey);
		...
		if (!res.commandResults || !res.commandResults[0] || !res.commandResults[0].returnValues) {
			throw new Error(`Failed to get margin manager BYL balance: Unknown error`);
		}
