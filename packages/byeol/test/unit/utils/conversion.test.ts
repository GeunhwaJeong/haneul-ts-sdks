...
const BYL_SCALAR = 1_000_000; // 1e6
...
describe('convertQuantity', () => {
	...
	it('handles small fractional numbers', () => {
		expect(convertQuantity(0.001, BYL_SCALAR)).toBe(1_000n);
	});
	...
	it('rounds fractional results via Math.round', () => {
		...
		expect(convertQuantity(1.0000005, BYL_SCALAR)).toBe(1_000_001n);
	});
});
...
describe('convertPrice', () => {
	it('scales number input with cross-scalar formula', () => {
		...
		expect(convertPrice(1.5, FLOAT_SCALAR, BYL_SCALAR, FLOAT_SCALAR)).toBe(1_500_000n);
	});

	it('passes bigint through without scaling', () => {
		expect(convertPrice(1_500_000n, FLOAT_SCALAR, BYL_SCALAR, FLOAT_SCALAR)).toBe(1_500_000n);
	});

	it('handles equal base/quote scalars (stable pair)', () => {
		// price=1.0 with same scalar: Math.round((1.0 * 1e9 * 1e6) / 1e6) = 1000000000
		expect(convertPrice(1.0, FLOAT_SCALAR, BYL_SCALAR, BYL_SCALAR)).toBe(1_000_000_000n);
	});

	it('returns 0n for number zero', () => {
		expect(convertPrice(0, FLOAT_SCALAR, BYL_SCALAR, FLOAT_SCALAR)).toBe(0n);
	});
});
