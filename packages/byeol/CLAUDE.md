# CLAUDE.md - Byeol Package

This file contains package-specific guidance for Claude Code when working with the byeol
package.
...
Byeol is a decentralized exchange (DEX) SDK for Haneul blockchain. It provides client extensions
for interacting with Byeol pools, margin managers, and flash loans.
...
packages/byeol/
├── src/
│   ├── index.ts           # Main entry point, exports byeol() extension + all public types
│   ├── client.ts          # ByeolClient — thin delegate to query modules + contract properties
...
Byeol uses the Haneul client extension pattern via `$extend()`:
...
// Access Byeol methods via client.byeol.*
await client.byeol.getLevel2Range('HANEUL_USDC', ...);
```
...
```typescript
const states = await client.byeol.getMarginManagerStates({
...
const priceInfoObjects = await client.byeol.getPriceInfoObjects(priceUpdateTx, [
	'HANEUL',
	'USDC',
	'BYL',
	'WBTC',
...
```bash
# Run tests
pnpm --filter @haneullabs/byeol test

# Run codegen (requires ../byeol sibling repo)
...
| -------------------- | -------------------- | ------------------------ | -------------- |
| `testnetCoins`       | `CoinMap`            | Uppercase coin symbol    | `BYL`, `HANEUL`  |
| `mainnetCoins`       | `CoinMap`            | Uppercase coin symbol    | `USDC`, `XBTC` |
| `testnetPools`       | `PoolMap`            | `BASE_QUOTE` (uppercase) | `BYL_HANEUL`     |
| `mainnetPools`       | `PoolMap`            | `BASE_QUOTE` (uppercase) | `HANEUL_USDC`     |
| `testnetMarginPools` | `MarginPoolMap`      | Uppercase coin symbol    | `HANEUL`, `BYL`  |
| `mainnetMarginPools` | `MarginPoolMap`      | Uppercase coin symbol    | `HANEUL`, `USDC`  |
| `testnetPackageIds`  | `ByeolPackageIds` | Fixed keys               | —              |
| `mainnetPackageIds`  | `ByeolPackageIds` | Fixed keys               | —              |
...
contract builders, and the user's address. `ByeolClient` delegates to these modules:
...
- Requires sibling repo `../byeol` for codegen
...
```
## @haneullabs/byeol Changes Summary
