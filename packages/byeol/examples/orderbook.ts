...
(async () => {
	...
	const bids = await client.byeol.getLevel2Range(poolKey, 0.01, Number(mid), true);
	console.log(bids);

	// 3. Level 2 order book - asks
	console.log('\nTop asks (price range mid to 1000):');
	const asks = await client.byeol.getLevel2Range(poolKey, Number(mid), 1000, false);
	console.log(asks);

	// 4. Level 2 ticks from mid
	console.log('\n5 ticks from mid:');
	const ticks = await client.byeol.getLevel2TicksFromMid(poolKey, 5);
	console.log(ticks);
