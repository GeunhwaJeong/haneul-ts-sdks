...
(async () => {
	...
	const pools = ['HANEUL_USDC', 'BYL_HANEUL', 'BYL_USDC', 'WUSDT_USDC', 'WUSDC_USDC', 'BETH_USDC']; // Live pools, add more if needed
	console.log('Manager:', manager);

	for (const pool of pools) {
		console.log(pool);
		console.log(await client.byeol.accountOpenOrders(pool, manager));
	}
})();
