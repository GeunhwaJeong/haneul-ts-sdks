...
(async () => {
	...
	const pools = ['HANEUL_USDC', 'BYL_HANEUL', 'BYL_USDC', 'WUSDT_USDC', 'WUSDC_USDC', 'BETH_USDC']; // Update pools as needed
	const manager = 'MANAGER_1'; // Update the manager accordingly
	console.log('Manager:', manager);
	for (const pool of pools) {
		const orders = await client.byeol.accountOpenOrders(pool, manager);
		const bidOrdersMap = new Map<number, number>();
		const askOrdersMap = new Map<number, number>();

		for (const orderId of orders) {
			const order = await client.byeol.getOrderNormalized(pool, orderId);
			if (!order) {
				continue;
			}
