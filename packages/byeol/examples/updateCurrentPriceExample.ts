 ...
 * 1. Batch update Pyth price feeds for all 4 mainnet assets (HANEUL, USDC, BYL, WAL)
 * 2. Update the current price for HANEUL_USDC, BYL_USDC, and WAL_USDC pools
 *
 * Usage:
...
(async () => {
	...
	const coinKeys = ['HANEUL', 'USDC', 'BYL', 'WAL'];

	// Pools to update current price for
	const poolKeys = ['HANEUL_USDC', 'BYL_USDC', 'WAL_USDC'];
	...
	try {
		...
		const priceInfoObjects = await client.byeol.getPriceInfoObjects(tx, coinKeys);
		...
		for (const poolKey of poolKeys) {
			client.byeol.poolProxy.updateCurrentPrice(poolKey)(tx);
