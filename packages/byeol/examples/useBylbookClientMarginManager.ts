...
(async () => {
	...
	try {
		...
		const owner = await client.byeol.getMarginManagerOwner(marginManagerKey);
		console.log('Owner:', owner);
		console.log('');

		// Test 2: Get Byeol Pool
		console.log('2. Margin Manager Byeol Pool:');
		const byeolPool = await client.byeol.getMarginManagerByeolPool(marginManagerKey);
		console.log('Byeol Pool ID:', byeolPool);
		console.log('');

		// Test 3: Get Margin Pool ID
		console.log('3. Margin Manager Margin Pool ID:');
		const marginPoolId = await client.byeol.getMarginManagerMarginPoolId(marginManagerKey);
		...
		const borrowedShares = await client.byeol.getMarginManagerBorrowedShares(marginManagerKey);
		...
		const borrowedBaseShares =
			await client.byeol.getMarginManagerBorrowedBaseShares(marginManagerKey);
		...
		const borrowedQuoteShares =
			await client.byeol.getMarginManagerBorrowedQuoteShares(marginManagerKey);
		...
		const hasBaseDebt = await client.byeol.getMarginManagerHasBaseDebt(marginManagerKey);
		...
		const balanceManagerId =
			await client.byeol.getMarginManagerBalanceManagerId(marginManagerKey);
		...
		const assets = await client.byeol.getMarginManagerAssets(marginManagerKey, decimals);
		...
		const debts = await client.byeol.getMarginManagerDebts(marginManagerKey, decimals);
