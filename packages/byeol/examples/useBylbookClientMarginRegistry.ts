...
(async () => {
	...
	try {
		...
		const isEnabled = await client.byeol.isPoolEnabledForMargin(poolKey);
		...
		const marginManagerIds = await client.byeol.getMarginManagerIdsForOwner(testOwner);
		...
		const baseMarginPoolId = await client.byeol.getBaseMarginPoolId(poolKey);
		console.log('Base Margin Pool ID:', baseMarginPoolId);
		console.log('');

		// Test 4: Get quote margin pool ID
		console.log('4. Quote Margin Pool ID:');
		const quoteMarginPoolId = await client.byeol.getQuoteMarginPoolId(poolKey);
		...
		const minWithdrawRatio = await client.byeol.getMinWithdrawRiskRatio(poolKey);
		...
		const minBorrowRatio = await client.byeol.getMinBorrowRiskRatio(poolKey);
		...
		const liquidationRatio = await client.byeol.getLiquidationRiskRatio(poolKey);
		...
		const targetLiquidationRatio = await client.byeol.getTargetLiquidationRiskRatio(poolKey);
		...
		const userReward = await client.byeol.getUserLiquidationReward(poolKey);
		...
		const poolReward = await client.byeol.getPoolLiquidationReward(poolKey);
		...
		const maintainers = await client.byeol.getAllowedMaintainers();
		...
		const pauseCaps = await client.byeol.getAllowedPauseCaps();
