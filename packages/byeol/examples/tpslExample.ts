 ...
 * in the Byeol margin trading system.
 *
...
import { byeol, OrderType, SelfMatchingOptions } from '../src/index.js';
...
(async () => {
	...
	try {
		const conditionalOrderIds = await client.byeol.getConditionalOrderIds('MARGIN_MANAGER_1');
	...
	try {
		const lowestTriggerAbove = await client.byeol.getLowestTriggerAbovePrice('MARGIN_MANAGER_1');
	...
	try {
		const highestTriggerBelow =
			await client.byeol.getHighestTriggerBelowPrice('MARGIN_MANAGER_1');
	...
	try {
		const managerState = await client.byeol.getMarginManagerState('MARGIN_MANAGER_1');
	...
	await client.byeol.getPriceInfoObject(tx, 'HANEUL');
	await client.byeol.getPriceInfoObject(tx, 'DBUSDC');
	...
	client.byeol.marginTPSL.addConditionalOrder({
	...
	client.byeol.marginTPSL.cancelConditionalOrder('MARGIN_MANAGER_1', '1002')(tx);
	...
	client.byeol.marginTPSL.cancelAllConditionalOrders('MARGIN_MANAGER_1')(tx);
	...
	client.byeol.marginTPSL.executeConditionalOrders(targetManagerAddress, 'HANEUL_USDC', 10)(tx);
	...
	await client.byeol.getPriceInfoObject(tx2, 'HANEUL');
	await client.byeol.getPriceInfoObject(tx2, 'DBUSDC');

	// 3.1 Create a condition manually
	const condition = client.byeol.marginTPSL.newCondition(
	...
	const pendingLimitOrder = client.byeol.marginTPSL.newPendingLimitOrder('HANEUL_DBUSDC', {
		clientOrderId: '3001',
	...
	const pendingMarketOrder = client.byeol.marginTPSL.newPendingMarketOrder('HANEUL_DBUSDC', {
		clientOrderId: '3002',
		selfMatchingOption: SelfMatchingOptions.SELF_MATCHING_ALLOWED,
		quantity: 2,
