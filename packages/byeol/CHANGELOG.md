...
- e68bd36: Align margin SDK with `byeol_margin` v5 on-chain source:
  ...
  - Add `ByeolAdminContract.mintCorePauseCap`, `revokeCorePauseCap`,
...
- bb8d26a: Fix three latent type errors in the generated `utils/index.ts` that surfaced for
  ...
  All consumer packages (`payment-kit`, `pas`, `walrus`, `suins`, `byeol`, `kiosk`) have been
...
- 993aa1f: Add `cancelLiveOrder` and `cancelLiveOrders` transaction builders that skip order ids not
  ...
  not owned by this BM) instead of aborting. Also updates mainnet `BYEOL_PACKAGE_ID` to
...
- e00788c: ### Breaking Changes
  ...
  **Renamed `env` option to `network`**
  ...
  ```diff
  const dbClient = new ByeolClient({
     address: '0x...',
  -  env: 'mainnet',
  +  network: 'mainnet',
     client: suiClient,
  ...
  **Removed `Environment` type export**
  ...
  ```diff
  -import { Environment } from '@mysten/byeol';
  ...
  **Client extension auto-detects network**
  ...
  The `byeol()` client extension function no longer accepts a `network` option. The network is
  ...
  **Network validation**
  ...
  The SDK now throws an error if the network is not `'mainnet'` or `'testnet'`, as Byeol only
...
- 4ace43a: Update BYL testnet feed
...
- 3fff516: Byeol Core Package Upgrade (V4)
...
- e0490df: Fix byeol admin function
...
- c95ff1a: Update Byeol and margin package IDs
...
- 7923ed5: Newest byeol package constants
...
- 05fb3ac: Update byeol addresses
