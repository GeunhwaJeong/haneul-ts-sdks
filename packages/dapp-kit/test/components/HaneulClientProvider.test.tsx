...
describe('HaneulClientProvider', () => {
	...
	it('provides a HaneulJsonRpcClient instance to its children', () => {
		const ChildComponent = () => {
			const client = useSuiClient();
			expect(client).toBeInstanceOf(HaneulJsonRpcClient);
			return <div>Test</div>;
		};
	...
	it('can accept pre-configured HaneulClients', () => {
		const suiClient = new HaneulJsonRpcClient({ url: 'http://localhost:8080', network: 'localnet' });
		const ChildComponent = () => {
			const client = useSuiClient();
			expect(client).toBeInstanceOf(HaneulJsonRpcClient);
		...
		render(
			<HaneulClientProvider networks={{ localnet: suiClient }}>
				<ChildComponent />
			</HaneulClientProvider>,
		);
