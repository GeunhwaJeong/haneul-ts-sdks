...
description: Sign Haneul transactions with a key stored in AWS Key Management Service
keywords: [AWS KMS, signer, AwsKmsSigner, KMS, Ed25519, Secp256k1, Secp256r1, Haneul, TypeScript SDK]
---

The `AwsKmsSigner` signs Haneul transactions using a key held in
...
`Secp256k1`, `ECC_NIST_P256` → `Secp256r1`). `Ed25519` is Haneul's native scheme.
...
```sh npm2yarn
npm i @haneullabs/aws-kms-signer
```

## Creating a signer

Construct the signer with `AwsKmsSigner.fromKeyId`, passing the KMS key ID and the AWS credentials
and region. This is async: it fetches the public key from KMS so the signer can derive the Haneul
address.

```typescript
import { AwsKmsSigner } from '@haneullabs/aws-kms-signer';
...
Once created, the signer behaves like any other [`Signer`](/haneul/cryptography): derive the address,
...
See [Cryptography](/haneul/cryptography) for the full signing and verification API shared by all
signers.
