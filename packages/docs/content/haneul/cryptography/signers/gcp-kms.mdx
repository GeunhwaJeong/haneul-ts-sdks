...
description: Sign Haneul transactions with a key stored in Google Cloud Key Management Service
keywords:
  [GCP KMS, signer, GcpKmsSigner, KMS, Secp256k1, Secp256r1, Google Cloud, Haneul, TypeScript SDK]
---

The `GcpKmsSigner` signs Haneul transactions using a key held in
...
```sh npm2yarn
npm i @haneullabs/gcp-kms-signer
```
...
```typescript
import { GcpKmsSigner } from '@haneullabs/gcp-kms-signer';
