---
title: HaneulGrpcClient
description: Connect to Haneul through gRPC with HaneulGrpcClient.
keywords:
  [
    HaneulGrpcClient,
    gRPC,
...
The `HaneulGrpcClient` provides access to the Full Node gRPC API.

For more complete details on what is available through this API see the
[gRPC API docs](https://docs.haneul.io/develop/accessing-data/grpc/what-is-grpc).

## Creating a gRPC client

To get started, create a `HaneulGrpcClient` instance by specifying a network and base URL:
...
By default, `HaneulGrpcClient` uses `GrpcWebFetchTransport` from
...
re-exported from `@haneullabs/haneul/grpc` for convenience.
...
```typescript
import { HaneulGrpcClient, GrpcWebFetchTransport } from '@haneullabs/haneul/grpc';
...
const transport = new GrpcTransport({
	host: 'fullnode.testnet.haneul.io:443',
...
The `HaneulGrpcClient` exposes several service clients for lower-level access to the gRPC API. These
service clients are generated using [protobuf-ts](https://github.com/timostamm/protobuf-ts), which
provides type-safe gRPC clients for TypeScript. For more details on how to use gRPC with Haneul, see
the [gRPC overview](https://docs.haneul.io/develop/accessing-data/grpc/what-is-grpc).
