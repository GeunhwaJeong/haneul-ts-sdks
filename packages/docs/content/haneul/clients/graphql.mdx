...
The `SuiGraphQLClient` enables type-safe GraphQL queries against the Haneul GraphQL API.

For more details on the Haneul GraphQL API, see the
[GraphQL reference](https://docs.haneul.io/references/haneul-graphql).

The `SuiGraphQLClient` implements all the the [Core API](/haneul/clients/core) methods:
