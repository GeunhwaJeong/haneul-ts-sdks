---
title: Core API
description: Transport-agnostic Core API shared by all Haneul clients
---

The Core API is the transport-agnostic interface that all Haneul clients implement. It provides a
consistent set of methods for interacting with the Haneul blockchain, regardless of whether you're
...
| `display`             | `boolean` | [Haneul Display Standard](https://docs.haneul.io/standards/display) metadata    |
...
console.log(object.type); // e.g., "0x2::coin::Coin<0x2::haneul::HANEUL>"
```
...
const result = await client.core.listOwnedObjects({
	owner: '0xabc...',
	filter: {
		StructType: '0x2::coin::Coin<0x2::haneul::HANEUL>',
	},
	limit: 10,
});
...
generated types (from `@haneullabs/codegen`) or manual BCS definitions:
...
`content`. If you do need `objectBcs`, parse it with `bcs.Object` from `@haneullabs/haneul/bcs`:
...
The `display` option fetches [Haneul Display Standard](https://docs.haneul.io/standards/display) metadata
...
const balance = await client.core.getBalance({
	owner: '0xabc...',
	coinType: '0x2::haneul::HANEUL', // Optional, defaults to HANEUL
});
...
Resolve an address to its default HaneulNS name.
...
console.log(name); // e.g., "example.haneul"
```
...
const { type } = await client.core.mvr.resolveType({
	type: '@haneullabs/haneul::coin::Coin<@haneullabs/haneul::haneul::HANEUL>',
});

console.log(type); // "0x2::coin::Coin<0x2::haneul::HANEUL>"
```
...
## HaneulClientTypes Namespace

The `HaneulClientTypes` namespace contains all type definitions for the Core API. Import it when you
...
function processObject(obj: HaneulClientTypes.Object<{ content: true }>) {
...
async function fetchBalance(
	...
	owner: string,
): Promise<HaneulClientTypes.CoinBalance> {
	const { balance } = await client.core.getBalance({ owner });
	return balance;
}

// Type options
const options: HaneulClientTypes.GetObjectOptions<{ content: true }> = {
	objectId: '0x123...',
	include: { content: true },
};
```
