...
<Callout type="warn">
	...
	[SuiGrpcClient](/haneul/clients/grpc) or [SuiGraphQLClient](/haneul/clients/graphql) as soon as
	possible.
</Callout>

The `SuiJsonRpcClient` connects to a Haneul network's JSON-RPC server. It implements the
...
To establish a connection to a network, import `SuiJsonRpcClient` from `@haneullabs/haneul/client` and pass
...
```typescript
import { getJsonRpcFullnodeUrl, SuiJsonRpcClient } from '@haneullabs/haneul/jsonRpc';
