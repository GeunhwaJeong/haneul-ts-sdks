---
title: HaneulJsonRpcClient
description: Connect to Haneul through JSON-RPC with HaneulJsonRpcClient.
keywords:
  [
    HaneulJsonRpcClient,
    JSON-RPC,
...
<Callout type="warn">
	The Haneul JSON-RPC API has been deprecated. We recommend migration to
	[`HaneulGrpcClient`](/haneul/clients/grpc) or [`HaneulGraphQLClient`](/haneul/clients/graphql) as soon as
	possible.
</Callout>

The `HaneulJsonRpcClient` connects to a Haneul network's JSON-RPC server. It implements the
[Core API](/haneul/clients/core), so it can be used with any SDK that accepts `ClientWithCoreApi`.
...
To establish a connection to a network, import `HaneulJsonRpcClient` from `@haneullabs/haneul/client` and pass
...
const client = new HaneulJsonRpcClient({ url: rpcUrl, network: 'devnet' });
...
You can use `HaneulJsonRpcClient` to call any RPC method the node you're connecting to exposes. Most
RPC methods are built into `HaneulJsonRpcClient`, but you can use `call` to leverage any methods
...
The `HaneulJsonRpcClient` uses a `Transport` class to manage connections to the RPC node. The default
`HaneulHTTPTransport` (alias for `JsonRpcHTTPTransport`) makes both JSON RPC requests, as well as
...
```typescript
import { HaneulJsonRpcClient, JsonRpcHTTPTransport } from '@haneullabs/haneul/jsonRpc';
...
const client = new HaneulJsonRpcClient({
	transport: new JsonRpcHTTPTransport({
		...
		websocket: {
			...
			url: 'wss://fullnode.devnet.haneul.io:443',
		},
		rpc: {
			headers: {
...
`HaneulJsonRpcClient` exposes a number of RPC methods that return paginated results. These methods
...
In addition to the RPC methods mentioned above, `HaneulJsonRpcClient` also exposes some methods for
