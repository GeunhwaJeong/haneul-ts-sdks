...
- `JsonRpcProvider` - The `JsonRpcProvider` class is deprecated in favor of the `suiClient` class
  ...
  [Network Interactions with SuiJsonRpcClient](/haneul/clients) for more information.
...
SuiJsonRpcClient The ability to request HANEUL from a faucet is not part of `SuiJsonRpcClient` as it
...
```ts
import { getFaucetHost, requestSuiFromFaucetV0 } from '@haneullabs/haneul/faucet';
