...
description: Migrate from JSON-RPC to the new Core API using HaneulGrpcClient or HaneulGraphQLClient.
keywords:
  [
    migration,
    JSON-RPC,
...
<Callout type="info">
	We recommend using `HaneulGrpcClient` for most operations and `HaneulGraphQLClient` for complex queries
...
The gRPC client should work with almost all haneullabs SDKs as a drop in replacement for the JSON-RPC
...
| `getLatestHaneulSystemState`           | `ledgerService.getEpoch` with `system_state` in the read mask    |
...
`haneul_amount` per epoch) and estimate the annualized rate from the change in exchange rate over the
trailing epochs. Two concrete reference implementations:

- The `jsonrpc-alt` implementation in
  [`haneul-indexer-alt-jsonrpc`](https://github.com/GeunhwaJeong/haneul/blob/31537d4d9235b9f61dc07a3a71b05ed61a2bda7b/crates/haneul-indexer-alt-jsonrpc/src/api/governance.rs#L422-L440),
  which is compact and readable.
- [This GraphQL approach](https://github.com/GeunhwaJeong/haneul/issues/23832#issuecomment-4437791087),
...
const client = new HaneulGrpcClient({
	baseUrl: 'http://158.69.54.239:9000',
	network: 'mainnet',
}).$extend(byeol({ address: myAddress }), haneulns());
