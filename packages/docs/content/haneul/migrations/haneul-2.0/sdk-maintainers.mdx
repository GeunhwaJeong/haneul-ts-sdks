...
Accept `ClientWithCoreApi` instead of `HaneulClient` to support all 3 Haneul clients (JSON-RPC, GraphQL,
gRPC):
...
Use [`@haneullabs/codegen`](/codegen) to generate type-safe TypeScript bindings from your Move packages.
