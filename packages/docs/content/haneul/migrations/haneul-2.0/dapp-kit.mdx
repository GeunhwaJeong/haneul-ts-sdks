---
title: '@haneullabs/dapp-kit'
description: Migrate @haneullabs/dapp-kit to the new dapp-kit-react package in 2.0.
keywords:
  [
    migration,
    '@haneullabs/dapp-kit',
...
<Callout type="info">
	The legacy `@haneullabs/dapp-kit` package will continue to work with the latest SDK, but it only
...
In the old dApp kit, you could optionally pass a `chain` parameter (for example, `haneul:mainnet`) to
...
<Callout type="warning">
	...
	Standard](https://docs.haneul.io/standards/wallet-standard) and so the dApp kit provides no
	replacement.
</Callout>
