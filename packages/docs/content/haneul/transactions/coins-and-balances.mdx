...
Haneul has two systems for holding fungible token balances: coin objects and address balances.
...
// HANEUL (balance is in GEUNHWA — 1 HANEUL = 1,000,000,000 GEUNHWA)
...
| ------------ | ------------------ | ---------- | ------------------------------------------------------------------------ |
| `balance`    | `bigint \| number` | _required_ | Amount in base units (GEUNHWA for HANEUL)                                      |
| `type`       | `string`           | HANEUL        | Coin type. Defaults to `0x2::haneul::HANEUL`                                   |
| `useGasCoin` | `boolean`          | `true`     | For HANEUL, split from the gas coin. Set `false` for sponsored transactions |

For HANEUL, `tx.coin()` splits from the gas coin by default. For sponsored transactions where the gas
...
For non-HANEUL coin types, pass the `type` parameter:
...
gas fees in HANEUL. The sender does not need to hold HANEUL in their wallet. Gasless transactions are an
extension of
[address balances](https://docs.haneul.io/onchain-finance/asset-custody/address-balances/using-address-balances),
built around `0x2::balance::send_funds` with `gasPrice = 0` and `gasBudget = 0` on the transaction.
See the
[Haneul gasless stablecoin transfers guide](https://docs.haneul.io/develop/transaction-payment/gasless-stablecoin-transfers)
for full details, including the current
[allowlist of eligible stablecoins](https://docs.haneul.io/develop/transaction-payment/gasless-stablecoin-transfers#eligible-stablecoins).
...
const client = new HaneulGrpcClient({ url: 'https://grpc.mainnet.haneul.io:443' });

// Mainnet USDC. For the full set of allowlisted gasless stablecoins, see:
// https://docs.haneul.io/develop/transaction-payment/gasless-stablecoin-transfers#eligible-stablecoins
const USDC = '0x6b7638d3d91245229f51f48d5c39bcb6ff7023ff022954a0bd94441d8ee20b4a::usdc::USDC';
...
[allowlist](https://docs.haneul.io/develop/transaction-payment/gasless-stablecoin-transfers#eligible-stablecoins)
