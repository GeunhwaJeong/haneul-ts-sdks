...
Every interaction with the Haneul network goes through a transaction. Haneul transactions are a sequence
of commands called
[programmable transaction blocks (PTBs)](https://docs.haneul.io/guides/developer/transactions/prog-txn-blocks)
...
[Haneul documentation on PTBs](https://docs.haneul.io/guides/developer/transactions/prog-txn-blocks) and
[inputs and results](https://docs.haneul.io/concepts/transactions/inputs-and-results).
...
Use `tx.balance()` and `balance::send_funds` to send HANEUL; this deposits directly into the
...
tx.moveCall({
	...
	arguments: [tx.balance({ balance: 1n * GEUNHWA_PER_HANEUL }), tx.pure.address('0xRecipientAddress')],
});
```
...
```typescript
tx.transferObjects([tx.coin({ balance: 1n * GEUNHWA_PER_HANEUL })], '0xRecipientAddress');
```
...
For non-HANEUL tokens, use the same patterns for passing the coin type:
...
Use `moveCall` to call any function in a published Move package. The Haneul framework at `0x2` provides
...
The [`@haneullabs/codegen`](/codegen) package generates type-safe TypeScript functions from your Move
...
See [SDK Building](/haneul/sdk-building) for more information on building composable transaction
libraries.
