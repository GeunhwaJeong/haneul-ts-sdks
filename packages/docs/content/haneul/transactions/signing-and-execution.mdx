...
description: Sign transactions and execute them on the Haneul network
...
how transaction signing and finality work at the protocol level, see the Haneul documentation on
[transaction lifecycle](https://docs.haneul.io/concepts/transactions/transaction-lifecycle) and
[transaction auth](https://docs.haneul.io/concepts/transactions/transaction-auth).
...
const keypair = Ed25519Keypair.fromSecretKey('haneulprivkey1...');
const grpcClient = new HaneulGrpcClient({
	network: 'mainnet',
	baseUrl: 'http://158.69.54.239:9000',
});
...
`fromSecretKey` accepts a Bech32-encoded private key (`haneulprivkey1...`) or a raw 32-byte
...
- `Ed25519Keypair` from `@haneullabs/haneul/keypairs/ed25519`
- `Secp256k1Keypair` from `@haneullabs/haneul/keypairs/secp256k1`
- `Secp256r1Keypair` from `@haneullabs/haneul/keypairs/secp256r1`
- `PasskeyKeypair` from `@haneullabs/haneul/keypairs/passkey`
...
```tsx
import { useSignTransaction } from '@haneullabs/dapp-kit';
...
Every Haneul transaction costs gas. The SDK handles gas automatically in most cases. It sets the gas
...
// Override the simulated budget (in GEUNHWA)
...
`tx.gas` references the coin used for gas payment. You can split HANEUL from it, merge coins into it,
