...
keywords: [Sponsor validation, custom validation analyzers, transaction analyzer, balanceFlows, Haneul]
---
...
```ts
import { analyzers, createAnalyzer, createSponsor } from '@haneullabs/sponsor';
...
‡ Verifies (through `@haneullabs/haneul`'s `isValidTransactionSignature`) that **every** supplied user
