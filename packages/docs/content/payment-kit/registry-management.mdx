...
description: Create, configure, and manage payment registries and accepted coin types on Haneul.
keywords: [PaymentRegistry, registry management, coin types, expiration, onchain, Haneul, Payment Kit]
---
...
```ts
// Withdraw all HANEUL from the registry
...
const paymentTx = client.paymentKit.tx.processRegistryPayment({
	...
	sender: keypair.getPublicKey().toHaneulAddress(),
	registryName: registryName,
});
