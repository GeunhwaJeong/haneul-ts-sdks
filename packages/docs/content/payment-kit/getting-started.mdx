...
description: Install and set up Payment Kit for your Haneul application.
keywords: [Payment Kit, getting started, installation, setup, Haneul, TypeScript, payment integration]
---
...
This guide will help you set up and start using the Payment Kit SDK in your Haneul application.

## Prerequisites

Before you begin, ensure you have:

- A Haneul wallet with Testnet or Mainnet HANEUL tokens
- Basic understanding of TypeScript and the Haneul blockchain

## Installation

Install the Payment Kit SDK and the Haneul TypeScript SDK:
...
The Payment Kit SDK extends the standard `HaneulGrpcClient` to provide payment functionality. Here's
how to set it up:
...
// Create a Haneul client with Payment Kit extension
...
const tx = client.paymentKit.tx.processRegistryPayment({
	...
	coinType: '0x2::haneul::HANEUL', // Coin type (HANEUL in this case)
	amount: 1n * GEUNHWA_PER_HANEUL, // 1 HANEUL (in GEUNHWA)
	receiver,
	sender: sender,
});
...
2. **Coin Type**: The type of coin being transferred (HANEUL token)
3. **Amount**: The payment amount in the smallest unit (GEUNHWA for HANEUL)
...
Payment Kit supports any Haneul coin type. Here's how to process payments with custom coins:
...
```ts
import { createPaymentTransactionUri } from '@haneullabs/payment-kit';

// Create a payment URI
const paymentUri = createPaymentTransactionUri({
	receiverAddress: '0x123...abc',
	amount: 1000000000n, // 1 HANEUL in GEUNHWA
	coinType: '0x2::haneul::HANEUL',
	nonce: crypto.randomUUID(),
...
// Output: haneul:pay?receiver=0x123...abc&amount=1000000000&coinType=0x2%3A%3Ahaneul%3A%3AHANEUL&nonce=...&label=Coffee%20Payment&message=Payment%20for%20espresso&registry=my-registry
```

The URI parameters include:

- **receiver**: The Haneul address receiving the payment (required)
...
```ts
import { parsePaymentTransactionUri } from '@haneullabs/payment-kit';
