...
Function to create a Payment Kit client extension for `HaneulGrpcClient`.

```ts
paymentKit<Name extends string = 'paymentKit'>({
	name = 'paymentKit' as Name,
}): HaneulClientRegistration<PaymentKitCompatibleClient, Name, PaymentKitClient>
```
...
type ProcessRegistryPaymentOptions = {
	...
	coinType: string; // Coin type (e.g., '0x2::haneul::HANEUL')
...
follow the `haneul:pay` protocol and can be used to encode payment parameters for QR codes, deep links,
or other sharing mechanisms.

### URI format

Payment URIs follow the `haneul:pay` protocol:
...
| ---------- | --------------------------------------------------- |
| `receiver` | Recipient Haneul address                               |
| `amount`   | Payment amount in smallest unit (for example, GEUNHWA) |
| `coinType` | Coin type (for example, `0x2::haneul::HANEUL`)            |
...
type PaymentUriParams = {
	receiverAddress: string; // Recipient Haneul address
...
- `PaymentKitUriError` if registry ID is not a valid Haneul object ID
...
const uri =
	'haneul:pay?receiver=0x1234...&amount=1000000000&coinType=0x2::haneul::HANEUL&nonce=abc123&registry=my-registry';
...
console.log(params.coinType); // '0x2::haneul::HANEUL'
...
- `PaymentKitUriError` if URI doesn't start with `haneul:pay?`
...
Check the
[CHANGELOG](https://github.com/GeunhwaJeong/haneul-ts-sdks/blob/main/packages/payment-kit/CHANGELOG.md) for
updates.
