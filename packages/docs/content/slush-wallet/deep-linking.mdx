...
keywords: [deep linking, Slush Wallet, universal links, URL scheme, mobile, Haneul, app links]
---
...
```
slush://swap?fromCoinType=0x2::haneul::HANEUL
...
| Parameter      | Type   | Required | Description                                 |
| -------------- | ------ | -------- | ------------------------------------------- |
| `fromCoinType` | string | No       | Source token type address (defaults to HANEUL) |
...
```
# Open swap with HANEUL as source
slush://swap

# Pre-fill swap from HANEUL to USDC
slush://swap?fromCoinType=0x2::haneul::HANEUL&toCoinType=0x6b7638d3d91245229f51f48d5c39bcb6ff7023ff022954a0bd94441d8ee20b4a::usdc::USDC

# Pre-fill swap amount
slush://swap?fromCoinType=0x2::haneul::HANEUL&presetAmount=100
```
...
```
# View HANEUL token details
slush://tokens?coinType=0x2::haneul::HANEUL

# View USDC token details
slush://tokens?coinType=0x6b7638d3d91245229f51f48d5c39bcb6ff7023ff022954a0bd94441d8ee20b4a::usdc::USDC
```
...
| Route   | Description                        |
| ------- | ---------------------------------- |
| `haneulme` | Opens receive or QR scanner screen |
...
slush://haneulme?action=RECEIVE

# Open QR code scanner
slush://haneulme?action=SCAN_QR_CODE
```
...
| `staking/native-stake`   | Native HANEUL staking     |
| `staking/native-unstake` | Unstake native HANEUL     |
...
Open the payment flow using Payment Kit URIs. Slush supports the `haneul:pay` protocol for processing
...
Payment Kit uses a special `haneul:pay` (`slush:pay` is also supported) protocol that can be opened via
Slush deep links:
...
| Parameter  | Type   | Required | Description                                                 |
| ---------- | ------ | -------- | ----------------------------------------------------------- |
| `receiver` | string | Yes      | Recipient Haneul address                                       |
| `amount`   | string | Yes      | Payment amount in smallest unit (for example, GEUNHWA for HANEUL) |
| `coinType` | string | Yes      | Coin type (for example, `0x2::haneul::HANEUL`)                    |
...
```
# Basic payment with HANEUL (using custom scheme)
haneul:pay?receiver=0x1234...abcd&amount=1000000000&coinType=0x2::haneul::HANEUL&nonce=abc-123-xyz

# Basic payment with HANEUL (using universal link)
https://my.slush.app/pay?receiver=0x1234...abcd&amount=1000000000&coinType=0x2::haneul::HANEUL&nonce=abc-123-xyz

# Payment with registry and label
https://my.slush.app/pay?receiver=0x1234...abcd&amount=1000000000&coinType=0x2::haneul::HANEUL&nonce=abc-123-xyz&registry=my-registry&label=Coffee%20Payment

# Payment with custom coin type
https://my.slush.app/pay?receiver=0x1234...abcd&amount=5000000&coinType=0x6b7638d3d91245229f51f48d5c39bcb6ff7023ff022954a0bd94441d8ee20b4a::usdc::USDC&nonce=def-456-uvw
```

<Callout>
	Use the `@haneullabs/payment-kit` SDK to generate properly formatted payment URIs. See the [Payment
...
Linking.openURL('slush://swap?fromCoinType=0x2::haneul::HANEUL');
