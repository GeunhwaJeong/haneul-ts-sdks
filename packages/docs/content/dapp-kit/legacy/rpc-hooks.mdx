...
Load data from the Haneul RPC using the `useSuiClientQuery` hook. This hook is a wrapper around the
...
You can fetch a variable number of Haneul RPC queries using the `useSuiClientQueries` hook. This hook
...
```tsx
import { useSuiClientQueries } from '@haneullabs/dapp-kit';
...
```tsx
import { useSuiClientInfiniteQuery } from '@haneullabs/dapp-kit';
...
```tsx
import { useSuiClientMutation } from '@haneullabs/dapp-kit';
