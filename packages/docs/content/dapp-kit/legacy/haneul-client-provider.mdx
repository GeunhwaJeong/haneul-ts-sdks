...
description: React provider for managing the active HaneulJsonRpcClient in legacy dApp Kit.
keywords:
  [HaneulClientProvider, React provider, Haneul client, JSON RPC, network config, legacy, dApp Kit]
---

The `HaneulClientProvider` manages the active `HaneulJsonRpcClient` that hooks and components use in the
dApp Kit.
...
`HaneulClientProvider` accepts a list of network configurations to create `HaneulJsonRpcClient` instances
...
- `networks`: A map of networks you can use. The keys are the network names, and the values can be
  either a configuration object (`HaneulClientOptions`) or a `HaneulJsonRpcClient` instance.
...
- `createClient`: A callback when a new `HaneulJsonRpcClient` is created (for example, when the active
  network changes). It receives the network name and configuration object as arguments, returning a
  `HaneulJsonRpcClient` instance.
...
## `HaneulJsonRpcClient` customization

The following example demonstrates how to create a custom `HaneulJsonRpcClient`.

```tsx
import { HaneulClientProvider } from '@haneullabs/dapp-kit';
import { getJsonRpcFullnodeUrl, HaneulJsonRpcClient, HaneulHTTPTransport } from '@haneullabs/haneul/jsonRpc';
...
## Using the `HaneulJsonRpcClient` from the provider

To use the `HaneulJsonRpcClient` from the provider, import the `useHaneulClient` function from the
