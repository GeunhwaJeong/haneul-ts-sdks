...
keywords: [Vue, dApp Kit, web components, nanostores, Haneul, TypeScript, wallet connection]
---
...
```npm
npm i @haneullabs/dapp-kit-core @haneullabs/haneul @nanostores/vue
```
...
<script setup lang="ts">
import { useStore } from '@nanostores/vue';
import { Transaction, coinWithBalance } from '@haneullabs/haneul/transactions';
import { dAppKit } from './dapp-kit';
...
| Component      | Usage                                                                  |
| -------------- | ---------------------------------------------------------------------- |
| Connect Button | `<haneullabs-dapp-kit-connect-button :instance="dAppKit" />`               |
| Connect Modal  | `<haneullabs-dapp-kit-connect-modal :instance="dAppKit" :open="isOpen" />` |
...
| `$currentClient`  | HaneulClient for current network                     |
...
See the complete
[Vue example](https://github.com/GeunhwaJeong/haneul-ts-sdks/tree/main/packages/dapp-kit/examples/vue/simple)
on GitHub.
