...
keywords: [React, dApp Kit, getting started, Vite, TypeScript, wallet connection, Haneul]
---
...
```tsx
import { useCurrentAccount, useCurrentWallet, useCurrentNetwork } from '@haneullabs/dapp-kit-react';
...
```tsx
import { useDAppKit, useCurrentAccount } from '@haneullabs/dapp-kit-react';
...
| [`useCurrentClient`](/dapp-kit/react/hooks/use-current-client)       | HaneulClient for current network            |
...
See the complete
[React example](https://github.com/GeunhwaJeong/haneul-ts-sdks/tree/main/packages/dapp-kit/examples/react/simple)
on GitHub.
