...
description: Build Haneul apps with framework-agnostic core and React bindings.
keywords: [dApp Kit, Haneul SDK, React, wallet connection, TypeScript, blockchain, web components]
---

<Callout type="info">
	**Migrating from @haneullabs/dapp-kit?** If you're currently using the legacy `@haneullabs/dapp-kit`
	package, check out our [Migration Guide](/haneul/migrations/haneul-2.0/dapp-kit) to upgrade to the new
	packages with gRPC and GraphQL support.
</Callout>

The Haneul dApp Kit provides tools and components for building decentralized applications on the Haneul
network. The SDK consists of two packages that work together:

## Packages

### @haneullabs/dapp-kit-core
...
<Callout type="warning">
	**Deprecated JSON RPC Only**: This legacy package only works with the deprecated JSON RPC API and
	...
	`@haneullabs/dapp-kit-core` and `@haneullabs/dapp-kit-react`.
</Callout>
