...
The `createDAppKit` function is the foundation of your Haneul dApp. It creates an instance that manages
wallet connections, network configuration, and provides access to the Haneul client.
...
import type { UiWallet, UiWalletAccount } from '@haneullabs/dapp-kit-core';
...
export const dAppKit = createDAppKit({
	...
	createClient: () => new HaneulGrpcClient({ network: 'localnet', baseUrl: 'http://127.0.0.1:9000' }),
...
export const dAppKit = createDAppKit({
	networks: ['mainnet'],
	createClient: () =>
		new HaneulGrpcClient({ network: 'mainnet', baseUrl: 'https://fullnode.mainnet.haneul.io:443' }),
	autoConnect: false,
});
```
