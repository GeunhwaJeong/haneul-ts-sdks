...
description: Sign and execute a transaction on the Haneul network using the connected wallet.
keywords: [sign and execute, transaction, dApp Kit, Haneul network, wallet, TypeScript, transfer]
---

The `signAndExecuteTransaction` action prompts the connected wallet to sign and immediately execute
a transaction on the Haneul network. This is the most common way to execute transactions in your app.
...
- The action transparently handles both `haneul:signAndExecuteTransaction` and
  `haneul:signAndExecuteTransactionBlock` wallet-standard features
