...
You can fetch a variable number of Sui RPC queries using the `useHaneulClientQueries` hook. This hook
...
To get the HaneulNS name for a given address, use the `useResolveSuiNSName` hook.

```tsx
import { useResolveSuiNSName } from '@haneullabs/dapp-kit';
