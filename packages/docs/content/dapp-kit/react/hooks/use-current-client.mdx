...
description: React hook to get the current Haneul client instance for the active network.
keywords:
  [useCurrentClient, React hook, Haneul client, blockchain client, dApp Kit, network, TypeScript]
---
...
<Callout type="info">
	By default, this returns a `HaneulClient` instance when using the standard setup. However, the dApp
	Kit supports any client that implements the Haneul core API, making it flexible for different
