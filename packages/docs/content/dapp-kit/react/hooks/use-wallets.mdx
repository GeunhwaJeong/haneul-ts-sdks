---
title: useWallets
description: React hook to list all available Haneul wallets detected by dApp Kit.
keywords: [useWallets, React hook, available wallets, wallet detection, dApp Kit, Haneul]
---

The `useWallets` hook provides access to all available wallets.

<Callout type="warn">
	All dApp Kit hooks must be used within components that are descendants of
	[`DAppKitProvider`](../dapp-kit-provider). Using them outside will result in an error.
</Callout>

## Usage

```tsx
import { useWallets } from '@haneullabs/dapp-kit-react';

export function MyComponent() {
	const wallets = useWallets();

	return (
		<div>
			{wallets.map((wallet) => (
				<div key={wallet.name}>
					<img src={wallet.icon} alt={wallet.name} />
					<span>{wallet.name}</span>
				</div>
			))}
		</div>
	);
}
```

## Return value

```tsx
UiWallet[]
```
