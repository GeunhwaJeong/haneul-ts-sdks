---
title: Connect Modal
description: Framework-agnostic web component for wallet selection and connection in Haneul dApps.
keywords:
  [
    connect modal,
    web component,
...
The `<haneullabs-dapp-kit-connect-modal>` Web Component provides a modal dialog for wallet connection.
...
<haneullabs-dapp-kit-connect-modal></haneullabs-dapp-kit-connect-modal>
...
<script>
	...
	const modal = document.querySelector('haneullabs-dapp-kit-connect-modal');
...
The `<haneullabs-dapp-kit-connect-modal>` Web Component can be only client-side rendered, as wallets are
