...
description:
  Framework-agnostic web component providing a complete wallet connection UI for Haneul dApps.
keywords:
  [
    connect button,
    web component,
...
The `<haneullabs-dapp-kit-connect-button>` Web Component provides a complete wallet connection UI. It
...
```html
<haneullabs-dapp-kit-connect-button></haneullabs-dapp-kit-connect-button>

<script>
	const button = document.querySelector('haneullabs-dapp-kit-connect-button');
...
`<haneullabs-dapp-kit-connect-modal>` component when using the connect button.
...
The `<haneullabs-dapp-kit-connect-button>` Web Component can be only client-side rendered, as wallets
