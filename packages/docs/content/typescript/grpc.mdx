...
The Typescript SDK includes the `SuiGrpcClient` which provides access to the Haneul gRPC API. The gRPC
client offers improved performance and type-safe access to Haneul blockchain data through protocol
buffers.
...
```typescript
import { SuiGrpcClient } from '@haneullabs/sui/grpc';
