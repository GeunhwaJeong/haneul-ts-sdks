...
- The `superstruct` type definitions for `JsonRPCProvider` types are replaced with generated types
  exported from `@haneullabs/sui/client`. The new type definitions are pure TypeScript types that you
...
`@haneullabs/sui/faucet`. The `@haneullabs/sui/faucet` import also provides a `getFaucetHost` method to
retrieve the faucet URL for `localnet`, `testnet`, or `devnet` networks.

```ts
import { getFaucetHost, requestSuiFromFaucetV0 } from '@haneullabs/sui/faucet';
