...
The majority of changes in this release are in the `@haneullabs/sui` package, but the changes to API
...
## Changes to `@haneullabs/sui`

As part of the 1.0 release the `@haneullabs/sui.js` package has been renamed to `@haneullabs/sui`

To upgrade to the new version, uninstall the old package and install the new one:

```sh npm2yarn
npm uninstall @haneullabs/sui.js
npm install @haneullabs/sui
```

Then update all imports to use the new package name:

```diff
- import { HaneulClient } from '@haneullabs/sui.js'
+ import { HaneulClient } from '@haneullabs/sui'
```

### `@haneullabs/sui/transactions`
...
```diff
- import { Transactions } from '@haneullabs/sui/transactions'
...
- [`TransactionObjectArgument`](/typedoc/types/_haneullabs_sui.transactions.TransactionObjectArgument.html)
  shape changed
- [`TransactionResult`](/typedoc/types/_haneullabs_sui.transactions.TransactionResult.html) shape
  changed
- [`TransactionObjectInput`](/typedoc/types/_haneullabs_sui.transactions.TransactionObjectInput.html)
  shape changed
- [`TransactionArgument`](/typedoc/types/_haneullabs_sui.transactions.TransactionArgument.html) shape
  changed
- [`TransactionBlockInput`](/typedoc/types/_haneullabs_sui.transactions.TransactionBlockInput.html)
  shape changed
...
The `publicKeyFromSuiBytes` has been moved to `@haneullabs/sui/verify`

```diff
- import { publicKeyFromSuiBytes } from '@haneullabs/sui/multisig'
