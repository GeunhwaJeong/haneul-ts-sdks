...
This basic example introduces you to the Haneul TypeScript SDK. The Node.js example mints SUI on a Haneul
...
[Sui Wallet browser extension](https://docs.haneullabs.com) to create one.
...
```sh npm2yarn
npm i -D @haneullabs/sui
```

Your `package.json` file now has a _dependencies_ section with `@haneullabs/sui` listed with the package
version number.

```json
"dependencies": {
    "@haneullabs/sui": "^<VERSION_NUMBER>"
}
```
...
const suiClient = new HaneulClient({ url: getFullnodeUrl('devnet') });
...
the faucet. You can check the total SUI for your address using the Haneul Wallet or Sui Client CLI.
