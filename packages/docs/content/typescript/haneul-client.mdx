...
To establish a connection to a network, import `SuiJsonRpcClient` from `@haneullabs/sui/jsonRpc` and
...
```typescript
import { SuiJsonRpcClient } from '@haneullabs/sui/jsonRpc';
...
Alternatively, you can use the `HaneulClient` alias from `@haneullabs/sui/client` for backward
compatibility:
...
```typescript
import { SuiJsonRpcClient, JsonRpcHTTPTransport } from '@haneullabs/sui/jsonRpc';
