...
<Callout type="warn">
	The Sui JSON RPC API has been deprecated, but is still heavily used across the Haneul ecosystem. A new version of the @haneullabs/haneul SDK will be released soon with better support for the [gRPC](./grpc) and [GraphQL](./graphql) APIs. Until this is ready, the `SuiJsonRpcClient` can still be used when working with SDKs that depend on `HaneulClient`.

    The [SuiGrpcClient](./grpc) and [HaneulGraphQLClient](./graphql) can already be used for most read use-cases and will be fully supported by all Mysten SDKs in the near future.

</Callout>

The Sui TypeScript SDK provides a `SuiJsonRpcClient` class (exported as `HaneulClient` for backward
compatibility) to connect to a network's JSON-RPC server.

<Callout type="info">
	Throughout the documentation, you may see references to `HaneulClient`. This is an alias for
	`SuiJsonRpcClient` and can be used interchangeably.
</Callout>

## Connecting to a Haneul network

To establish a connection to a network, import `SuiJsonRpcClient` from `@haneullabs/haneul/jsonRpc` and
...
Alternatively, you can use the `HaneulClient` alias from `@haneullabs/haneul/client` for backward
compatibility:
...
```typescript
import { SuiJsonRpcClient, JsonRpcHTTPTransport } from '@haneullabs/haneul/jsonRpc';
