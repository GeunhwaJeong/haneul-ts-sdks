...
The `@haneullabs/sui/bcs` package extends `@haneullabs/bcs` with Sui specific scheme definitions.

To learn more about using BCS see the [BCS documentation](/bcs).

the `bcs` export of `@haneullabs/sui/bcs` contains all the same exports as `bcs` from `@haneullabs/bcs` plus
